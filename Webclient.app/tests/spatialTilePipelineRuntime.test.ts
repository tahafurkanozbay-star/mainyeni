...
import {
  SpatialTilePipelineRuntime,
  type SpatialTilePipelineDescriptor,
} from '../src/gis-engine/spatialTilePipelineRuntime';

const tile = (
  key: string,
  layerId = 'parcels',
  overrides: Partial<SpatialTilePipelineDescriptor> = {},
): SpatialTilePipelineDescriptor => ({
  key,
  layerId,
  level: 12,
  row: 120,
...
describe('SpatialTilePipelineRuntime', () => {
  it('fails closed for malformed descriptors and subscriber identities', () => {
    ...
    expect(runtime.snapshot(1).rejected).toBe(4);
  ...
  it('completes a generation atomically and exposes a subsequent cache hit', () => {
    ...
    const cached = runtime.acquire(tile('complete'), 'map-b', 21);
    expect(cached).toMatchObject({ owner: false, state: 'cached', generation: 0 });
  ...
  it('does not count a cache hit against inflight request capacity', () => {
    ...
    expect(first).toBeDefined();
    runtime.complete('cached', first!.generation, cache(2));
  ...
  it('expires cache entries and reacquires transport work after TTL', () => {
    ...
    expect(first).toBeDefined();
    runtime.complete('ttl', first!.generation, { now: 2, ttlMs: 5, bytes: 1024 });
    expect(runtime.acquire(tile('ttl'), 'map-b', 3)?.state).toBe('cached');
    const reacquired = runtime.acquire(tile('ttl'), 'map-c', 8);
    expect(reacquired).toMatchObject({ owner: true, state: 'inflight' });
  });

  it('cancels only inflight work belonging to the selected layer', () => {
    const runtime = new SpatialTilePipelineRuntime();
    runtime.acquire(tile('parcel-a', 'parcels'), 'map', 1);
    runtime.acquire(tile('road-a', 'roads'), 'map', 2);
    expect(runtime.cancelLayer('parcels')).toEqual(['parcel-a']);
    expect(runtime.stateOf('parcel-a')).toBe('idle');
    expect(runtime.stateOf('road-a')).toBe('inflight');
  });

  it('preserves cached records when cancelling a layer', () => {
    const runtime = new SpatialTilePipelineRuntime();
    const lease = runtime.acquire(tile('cached-layer', 'parcels'), 'map', 1);
    expect(lease).toBeDefined();
    runtime.complete('cached-layer', lease!.generation, cache(2));
    expect(runtime.cancelLayer('parcels')).toEqual([]);
    expect(runtime.stateOf('cached-layer')).toBe('cached');
  });

  it('clear removes request, admission, cache and pipeline state deterministically', () => {
    const runtime = new SpatialTilePipelineRuntime();
    runtime.acquire(tile('z'), 'map', 1);
    runtime.acquire(tile('a'), 'map', 2);
    expect(runtime.clear()).toEqual(['a', 'z']);
    expect(runtime.snapshot(3)).toMatchObject({ knownTiles: 0, cacheEntries: 0, inflightRequests: 0 });
  });
  ...
  it('returns immutable diagnostics bounded by configured history capacity', () => {
    ...
    runtime.acquire(tile(''), 'map', 1);
    runtime.acquire(tile('a'), '', 2);
    runtime.acquire(tile('b'), '', 3);
    const diagnostics = runtime.diagnostics();
    expect(diagnostics).toHaveLength(2);
    expect(diagnostics.map((entry) => entry.sequence)).toEqual([1, 2]);
  ...
  it('forwards admission decisions through the bounded admission authority', () => {
    ...
    const first = runtime.admit({ key: 'a', layerId: 'parcels', estimatedBytes: 1024, priority: 'interactive' });
    const second = runtime.admit({ key: 'b', layerId: 'parcels', estimatedBytes: 1024, priority: 'prefetch' });
    expect(first).toBeDefined();
    expect(second).toBeDefined();
  ...
  it('forwards prefetch planning through the deterministic bounded planner', () => {
    ...
    const plan = runtime.planPrefetch([
      { key: 'near', layerId: 'roads', level: 12, row: 1, column: 1, estimatedBytes: 100, priority: 'nearby', distance: 10 },
      { key: 'far', layerId: 'roads', level: 12, row: 1, column: 2, estimatedBytes: 100, priority: 'speculative', distance: 100 },
    ]);
    expect(plan.selected).toHaveLength(1);
    expect(plan.selected[0]?.key).toBe('near');
  });

  it('reports deterministic state accounting across multiple layers', () => {
    const runtime = new SpatialTilePipelineRuntime();
    const cached = runtime.acquire(tile('cached', 'parcels'), 'map', 1);
    expect(cached).toBeDefined();
    runtime.complete('cached', cached!.generation, cache(2));
    runtime.acquire(tile('live', 'roads'), 'map', 3);
    expect(runtime.snapshot(3)).toMatchObject({
      knownTiles: 2,
      cachedTiles: 1,
      inflightTiles: 1,
      byLayer: { parcels: 1, roads: 1 },
      completed: 1,
    });
  });
  ...
  it('bounds subscriber identifiers before request allocation', () => {
    ...
    expect(runtime.acquire(tile('a'), '12345', 1)).toBeUndefined();
    expect(runtime.acquire(tile('b'), '1234', 2)).toBeDefined();
  });

  it('rejects non-finite acquisition timestamps without allocating state', () => {
    const runtime = new SpatialTilePipelineRuntime();
    expect(runtime.acquire(tile('a'), 'map', Number.NaN)).toBeUndefined();
    expect(runtime.acquire(tile('b'), 'map', Number.POSITIVE_INFINITY)).toBeUndefined();
    expect(runtime.snapshot(1).knownTiles).toBe(0);
  });

  it('returns idle for malformed or unknown state keys', () => {
    const runtime = new SpatialTilePipelineRuntime();
    expect(runtime.stateOf('')).toBe('idle');
    expect(runtime.stateOf('unknown')).toBe('idle');
  });
});
