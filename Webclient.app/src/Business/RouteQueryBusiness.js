...
export const RouteQueryBusiness={
    ...
    Query:async(_query, _returnGeometry)=>{
        ...
        return new Promise((resolve, reject) => {
            ...
            const queryServiceTitle="RouteQueryUrl";
            ...
            if (_query.showNearby) {
                ...
                if (_query != null) {
                    ...
                    if (!IsNull(_query.name)) {
                        options.where += " AND UPPER(adi) LIKE '%" + TextHelper.TurkishToUpper(_query.name) + "%'";
                    }
                    ...
                    if (!_query.showCultureWalkingRoute && IsNull(_query.Id)) {
                        options.where += " AND tip <> 1";
                    }
                    if (!_query.showNatureWalkingRoute && IsNull(_query.Id)) {
                        options.where += " AND tip <> 2";
                    }
                    
                    if(!IsNull(_query.routeLevel)){
                        options.where += " AND zorlukderecesi = " + _query.routeLevel;
                    }
                    

                    if (!IsNull(_query.Id)) {
                        options.where += " AND objectid = " + _query.Id;
                    }
                }
