...
describe('legacyPresentationRemediation', () => {
  test('proves the active repository legacy presentation risks are covered', () => {
    const report = evaluateLegacyPresentationRemediation({
      legacyCss,
      effectiveLegacyCss,
      foundationCss,
      modernizationCss,
  ...
  test('treats build-time remote font cleanup plus local typography as remediation', () => {
    const report = evaluateLegacyPresentationRemediation({
      legacyCss: riskyLegacy,
      effectiveLegacyCss: cleanEffectiveLegacy,
      foundationCss: minimalFoundation,
      modernizationCss: minimalModernization,
    });

    expect(report.risks['remote-font']).toMatchObject({
      status: 'remediated',
      findingCount: 1,
      missing: [],
    });
  ...
  test('fails closed if the effective build still includes a remote stylesheet', () => {
    const report = evaluateLegacyPresentationRemediation({
      legacyCss: riskyLegacy,
      effectiveLegacyCss: riskyLegacy,
      foundationCss: minimalFoundation,
      modernizationCss: minimalModernization,
    });
  ...
  test('requires a visible important focus ring to override legacy suppression', () => {
    const modernizationWithoutFocus = minimalModernization
      .replace(/:where\(button\):focus-visible[\s\S]*?\}\n/, '')
    ...
    const report = evaluateLegacyPresentationRemediation({
      ...
      effectiveLegacyCss: cleanEffectiveLegacy,
      foundationCss: minimalFoundation,
      modernizationCss: modernizationWithoutFocus,
    });
  ...
  test('requires keyboard-equivalent dropdown disclosure', () => {
    const report = evaluateLegacyPresentationRemediation({
      ...
      effectiveLegacyCss: cleanEffectiveLegacy,
      foundationCss: minimalFoundation,
      modernizationCss: minimalModernization.replace(
        '.dropdown:focus-within .dropdown-content { display: block; }',
        '',
      ),
    });
  ...
  test('requires both default and coarse-pointer target budgets', () => {
    const report = evaluateLegacyPresentationRemediation({
      legacyCss: riskyLegacy,
      effectiveLegacyCss: cleanEffectiveLegacy,
      foundationCss: minimalFoundation.replace('--legacy-control-height-coarse: 48px;', ''),
      modernizationCss: minimalModernization.replace('--legacy-target-coarse: 48px;', ''),
    });
  ...
  test('requires viewport-bounded logical query-window sizing', () => {
    const report = evaluateLegacyPresentationRemediation({
      ...
      foundationCss: minimalFoundation,
      modernizationCss: minimalModernization
        .replace('inline-size: min(34rem, calc(100dvw - 2rem));', 'width: 500px;')
        .replace('max-inline-size: var(--legacy-panel-max);', ''),
    });
  ...
  test('requires logical positioning evidence for legacy floating panels', () => {
    const report = evaluateLegacyPresentationRemediation({
      ...
      foundationCss: minimalFoundation,
      modernizationCss: minimalModernization
        .replace(/inset-inline-start:[^;]+;/g, '')
        .replace(/inset-inline-end:[^;]+;/g, '')
        .replace(/inset-block-start:[^;]+;/g, '')
        .replace(/inset-block-end:[^;]+;/g, ''),
    });
  ...
  test('does not count an unresolved remediation when the matching risk is absent', () => {
    ...
    const report = evaluateLegacyPresentationRemediation({
      legacyCss: cleanLegacy,
      effectiveLegacyCss: cleanLegacy,
      foundationCss: minimalFoundation,
      modernizationCss: '',
    });
  ...
  test('bounds oversized source input before auditing it', () => {
    const oversizedLegacy = `${'/* safe */\n'.repeat(80_000)}${riskyLegacy}`;
    const report = evaluateLegacyPresentationRemediation({
      legacyCss: oversizedLegacy,
      effectiveLegacyCss: cleanEffectiveLegacy,
      foundationCss: minimalFoundation,
      modernizationCss: minimalModernization,
    });
  ...
  test('never exposes unbounded legacy CSS in remediation evidence', () => {
    ...
    const report = evaluateLegacyPresentationRemediation({
      legacyCss: `${riskyLegacy}\n.bad { left: 1px; content: '${secretMarker.repeat(20)}'; }`,
      effectiveLegacyCss: cleanEffectiveLegacy,
      foundationCss: minimalFoundation,
      modernizationCss: minimalModernization,
    });
