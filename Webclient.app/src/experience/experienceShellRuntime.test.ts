...
beforeEach(() => {
  ...
  for (const keyName of Object.keys(document.documentElement.dataset)) {
    delete document.documentElement.dataset[keyName];
  }
...
describe("ExperienceShellRuntime", () => {
  ...
  test("reflects adaptive state to the document root", () => {
    ...
    expect(document.documentElement.style.getPropertyValue("--experience-panel-width")).toMatch(/px$/);
  ...
  test("focuses map with Alt+M", () => {
    ...
    expect(map.dataset.experienceTemporaryTabindex).toBe("true");
    runtime.dispose();
  ...
  test("tracks focused map scope", () => {
    ...
    const runtime = createExperienceShellRuntime({ window, document });
    map.focus();
    expect(runtime.snapshot.shortcuts.scope).toBe("map");
    runtime.dispose();
  });
  ...
  test("tracks focused panel scope", () => {
    ...
    const runtime = createExperienceShellRuntime({ window, document });
    button.focus();
    expect(runtime.snapshot.shortcuts.scope).toBe("panel");
    runtime.dispose();
  });
  ...
  test("dialog scope has priority over nested panel scope", () => {
    ...
    const runtime = createExperienceShellRuntime({ window, document });
    button.focus();
    expect(runtime.snapshot.shortcuts.scope).toBe("dialog");
    runtime.dispose();
  });

  test("recomputes compact bottom sheet policy after resize", () => {
    const runtime = createExperienceShellRuntime({ window, document });
    setWindowSize(520, 760);
    window.dispatchEvent(new Event("resize"));
    expect(runtime.snapshot.workspace.policy.viewport).toBe("compact");
    expect(runtime.snapshot.workspace.policy.placement).toBe("bottom-sheet");
    expect(document.documentElement.dataset.experienceViewport).toBe("compact");
    runtime.dispose();
  });
  ...
  test("uses visual viewport to detect virtual keyboard inset", () => {
    ...
    visual.width = 390;
    visual.height = 460;
    setWindowSize(390, 760);
    Object.defineProperty(window, "visualViewport", { configurable: true, value: visual });
    const runtime = createExperienceShellRuntime({ window, document });
    expect(runtime.snapshot.workspace.policy.keyboardInset).toBe(300);
    expect(document.documentElement.style.getPropertyValue("--experience-keyboard-inset")).toBe("300px");
    runtime.dispose();
  });
  ...
  test("updates when visual viewport changes", () => {
    ...
    Object.defineProperty(window, "visualViewport", { configurable: true, value: visual });
    const runtime = createExperienceShellRuntime({ window, document });
    expect(runtime.snapshot.workspace.policy.viewport).toBe("medium");
    visual.width = 600;
  ...
  test("propagates reduced motion into workspace transition policy", () => {
    ...
    expect(runtime.snapshot.accessibility.reducedMotion).toBe(true);
    expect(runtime.snapshot.workspace.policy.reducedMotion).toBe(true);
    expect(runtime.snapshot.workspace.policy.transitionMs).toBe(0);
    runtime.dispose();
  });

  test("propagates coarse pointer and hover capabilities", () => {
    const media = installMatchMedia();
    const coarse = media.get("(pointer: coarse)") ?? new FakeMediaQueryList("(pointer: coarse)");
    const hover = media.get("(hover: hover)") ?? new FakeMediaQueryList("(hover: hover)");
    media.set("(pointer: coarse)", coarse);
    media.set("(hover: hover)", hover);
    const runtime = createExperienceShellRuntime({ window, document });
    coarse.setMatches(true);
    hover.setMatches(false);
    expect(runtime.snapshot.accessibility.pointer).toBe("coarse");
    expect(runtime.snapshot.workspace.policy.inputMode).toBe("touch");
    expect(runtime.snapshot.workspace.policy.touchTarget).toBe(44);
    runtime.dispose();
  });
  ...
  test("supports explicit map mode changes", () => {
    ...
    expect(runtime.snapshot.workspace.state.mapMode).toBe("3d");
    expect(document.documentElement.dataset.experienceMapMode).toBe("3d");
    runtime.setMapMode("2d");
    expect(runtime.snapshot.workspace.state.mapMode).toBe("2d");
    runtime.dispose();
  });

  test("supports explicit shortcut scope changes", () => {
    const runtime = createExperienceShellRuntime({ window, document });
    runtime.setShortcutScope("dialog");
    expect(runtime.snapshot.shortcuts.scope).toBe("dialog");
    runtime.setShortcutScope("global");
    expect(runtime.snapshot.shortcuts.scope).toBe("global");
    runtime.dispose();
  });
  ...
  test("isolates observer failures through the shell error boundary", () => {
    ...
    expect(errors[0]?.source).toBe("observer");
    runtime.dispose();
  });

  test("returns false when requested focus target is absent", () => {
    const runtime = createExperienceShellRuntime({ window, document });
    expect(runtime.focusMap()).toBe(false);
    expect(runtime.focusPanel()).toBe(false);
    runtime.dispose();
  });
  ...
  test("reports invalid selectors without breaking the shell", () => {
    ...
    expect(runtime.focusMap()).toBe(false);
    expect(errors).toEqual(["shortcut"]);
    runtime.dispose();
  });

  test("refresh reads the latest media and viewport state", () => {
    const media = installMatchMedia();
    const runtime = createExperienceShellRuntime({ window, document });
    setWindowSize(800, 700);
    media.get("(forced-colors: active)")?.setMatches(true);
    runtime.refresh();
    expect(runtime.snapshot.workspace.policy.viewport).toBe("medium");
    expect(runtime.snapshot.accessibility.forcedColors).toBe(true);
    runtime.dispose();
  });
  ...
  test("dispose removes root reflection and prevents further mutations", () => {
    ...
    expect(document.documentElement.dataset.experienceViewport).toBeUndefined();
    expect(document.documentElement.style.getPropertyValue("--experience-panel-width")).toBe("");
    expect(() => runtime.setMapMode("3d")).toThrow(/dispose/);
  });
});
