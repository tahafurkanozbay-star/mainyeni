...
export class ExperienceShellRuntime {
  ...
  constructor(options: ExperienceShellRuntimeOptions = {}) {
    ...
    if (!runtimeWindow || !runtimeDocument) {
      throw new Error("ExperienceShellRuntime browser Window ve Document gerektirir.");
    }
  ...
  get snapshot(): Readonly<ExperienceShellSnapshot> {
  ...
  refresh(): Readonly<ExperienceShellSnapshot> {
    ...
    this.workspaceRuntime.setMetrics(
      readMetrics(this.runtimeWindow, this.accessibilityRuntime.snapshot),
    );
    return this.refreshSnapshot();
  }
  ...
  focusMap(): boolean {
    this.assertActive();
    return this.focusSelector(this.mapSelector);
  }

  focusPanel(): boolean {
    this.assertActive();
    return this.focusSelector(this.panelSelector);
  }
  ...
  private installRuntimeObservers(): void {
    ...
    const onFocusIn = (event: FocusEvent): void => {
      ...
      if (this.shortcutRuntime.snapshot.scope !== scope) {
        this.shortcutRuntime.setScope(scope);
      }
    };
  ...
  private installShortcuts(): void {
    this.releases.push(
      this.shortcutRuntime.register({
        id: "experience-focus-map",
        key: "m",
        alt: true,
        preventDefault: true,
        run: () => {
          this.focusSelector(this.mapSelector);
        },
      }),
      this.shortcutRuntime.register({
        id: "experience-focus-panel",
        key: "s",
        alt: true,
        preventDefault: true,
        run: () => {
          this.focusSelector(this.panelSelector);
        },
      }),
    );
  }
  ...
  private focusSelector(selector: string): boolean {
    ...
    try {
      node = this.runtimeDocument.querySelector(selector);
    } catch (error) {
      this.report(error, "shortcut");
      return false;
    }
  ...
  private readSize(): { width: number; height: number } {
    ...
    return {
      width: visual
        ? finiteNonNegative(visual.width, finiteNonNegative(this.runtimeWindow.innerWidth))
        : finiteNonNegative(this.runtimeWindow.innerWidth),
      height: visual
        ? finiteNonNegative(visual.height, finiteNonNegative(this.runtimeWindow.innerHeight))
        : finiteNonNegative(this.runtimeWindow.innerHeight),
    };
  }

  private buildSnapshot(width: number, height: number): ExperienceShellSnapshot {
    return Object.freeze({
      accessibility: this.accessibilityRuntime.snapshot,
      modality: this.modalityRuntime.snapshot,
      shortcuts: this.shortcutRuntime.snapshot,
      workspace: this.workspaceRuntime.getSnapshot(),
      width,
      height,
  ...
  private clearReflection(): void {
    ...
    for (const node of this.runtimeDocument.querySelectorAll<HTMLElement>(
      "[data-experience-temporary-tabindex='true']",
    )) {
  ...
  private notifyOne(
    listener: ExperienceShellListener,
    snapshot: Readonly<ExperienceShellSnapshot>,
    previous: Readonly<ExperienceShellSnapshot>,
  ): void {
    try {
  ...
  private assertActive(): void {
    if (this.disposed) {
      throw new Error("ExperienceShellRuntime dispose edildikten sonra kullanılamaz.");
    }
  }
}

export const createExperienceShellRuntime = (
  options: ExperienceShellRuntimeOptions = {},
): ExperienceShellRuntime => new ExperienceShellRuntime(options);
