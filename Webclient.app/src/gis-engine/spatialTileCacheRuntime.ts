 ...
 * It intentionally owns no network transport and stores no credentials or URLs.
 * Admission is byte-budgeted, expiration is deterministic, and eviction uses
 * least-recently-used ordering with insertion time and fingerprint tie breakers.
 */
export class SpatialTileCacheRuntime<T> {
  ...
  put(key: SpatialTileCacheKey, value: T, options: SpatialTileCachePutOptions): boolean {
    ...
    if (options.byteSize > this.#limits.maxEntryBytes || options.byteSize > this.#limits.maxBytes) {
      this.#rejected += 1;
      return false;
    }
    ...
    if (pinnedAfterReplace > this.#limits.maxPinnedEntries) {
      this.#rejected += 1;
      return false;
    }
  ...
  delete(key: SpatialTileCacheKey): boolean {
    const normalized = normalizeKey(key);
    return this.#remove(fingerprint(normalized), 'delete');
  }
  ...
  setPinned(key: SpatialTileCacheKey, pinned: boolean, now = Date.now()): boolean {
    ...
    if (!entry || this.#isExpired(entry, now)) {
      if (entry) this.#remove(id, 'expiration');
      return false;
    }
  ...
  entries(now = Date.now()): readonly SpatialTileCacheEntry<T>[] {
    ...
    return Object.freeze([...this.#entries.values()]
      .sort((a, b) => a.lastAccessedAt - b.lastAccessedAt || a.insertedAt - b.insertedAt || fingerprint(a.key).localeCompare(fingerprint(b.key)))
      .map((entry) => Object.freeze({ ...entry })));
  }

  snapshot(now = Date.now()): SpatialTileCacheSnapshot {
    this.pruneExpired(now);
    const entryRatio = this.#entries.size / this.#limits.maxEntries;
    const byteRatio = this.#bytes / this.#limits.maxBytes;
    const ratio = Math.max(entryRatio, byteRatio);
    const pressure: SpatialTileCachePressure = ratio >= 0.9 ? 'critical' : ratio >= 0.7 ? 'elevated' : 'normal';
  ...
  #evictToBudget(protectedId: string): boolean {
    while (this.#entries.size > this.#limits.maxEntries || this.#bytes > this.#limits.maxBytes) {
      const candidates = [...this.#entries.entries()]
        ...
        .sort(([idA, a], [idB, b]) => a.lastAccessedAt - b.lastAccessedAt || a.insertedAt - b.insertedAt || idA.localeCompare(idB));
