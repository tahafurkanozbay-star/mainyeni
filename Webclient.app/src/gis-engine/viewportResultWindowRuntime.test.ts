...
describe('viewportResultWindowRuntime', () => {
  ...
  it('replaces features across pages without increasing cardinality', () => {
    const runtime = createViewportResultWindowRuntime();
    runtime.commitPage({ windowKey: 'roads', generation: 1, pageIndex: 0, features: [{ id: 1, payload: 'old', estimatedBytes: 10 }] });
    const commit = runtime.commitPage({ windowKey: 'roads', generation: 1, pageIndex: 1, features: [{ id: 1, payload: 'new', estimatedBytes: 20 }] });
  ...
  it('treats already committed page indexes as idempotent duplicates', () => {
    const runtime = createViewportResultWindowRuntime();
    const page = { windowKey: 'roads', generation: 1, pageIndex: 0, features: [{ id: 1, payload: 'first', estimatedBytes: 10 }] };
    runtime.commitPage(page);
    const duplicate = runtime.commitPage({ ...page, features: [{ id: 2, payload: 'ignored', estimatedBytes: 10 }] });
  ...
  it('rejects stale generations without mutating the current window', () => {
    const runtime = createViewportResultWindowRuntime();
    runtime.commitPage({ windowKey: 'roads', generation: 5, pageIndex: 0, features: [{ id: 1, payload: 'current', estimatedBytes: 10 }] });
    const stale = runtime.commitPage({ windowKey: 'roads', generation: 4, pageIndex: 0, features: [{ id: 2, payload: 'stale', estimatedBytes: 10 }] });
  ...
  it('resets a window when a newer generation arrives', () => {
    const runtime = createViewportResultWindowRuntime();
    runtime.commitPage({ windowKey: 'roads', generation: 1, pageIndex: 0, features: [{ id: 1, payload: 'old', estimatedBytes: 10 }] });
    runtime.commitPage({ windowKey: 'roads', generation: 2, pageIndex: 0, features: [{ id: 2, payload: 'new', estimatedBytes: 10 }] });
  ...
  it('enforces per-window feature budgets transactionally', () => {
    const runtime = createViewportResultWindowRuntime({ maximumFeaturesPerWindow: 2, maximumTotalFeatures: 10 });
    runtime.commitPage({ windowKey: 'roads', generation: 1, pageIndex: 0, features: [{ id: 1, payload: 1 }, { id: 2, payload: 2 }] });
    expect(() => runtime.commitPage({ windowKey: 'roads', generation: 1, pageIndex: 1, features: [{ id: 3, payload: 3 }] })).toThrow(ViewportResultWindowError);
    expect(runtime.read('roads').map((feature) => feature.id)).toEqual([1, 2]);
  });

  it('enforces per-window byte budgets transactionally', () => {
    const runtime = createViewportResultWindowRuntime({ maximumBytesPerWindow: 20, maximumTotalBytes: 100 });
    runtime.commitPage({ windowKey: 'roads', generation: 1, pageIndex: 0, features: [{ id: 1, payload: 'a', estimatedBytes: 10 }] });
    expect(() => runtime.commitPage({ windowKey: 'roads', generation: 1, pageIndex: 1, features: [{ id: 2, payload: 'b', estimatedBytes: 11 }] })).toThrow('byte budget');
  ...
  it('enforces page budgets before mutation', () => {
    ...
    expect(() => runtime.commitPage({ windowKey: 'roads', generation: 1, pageIndex: 1, features: [] })).toThrow('page budget');
  });
  ...
  it('evicts least-recently-used unpinned windows under cardinality pressure', () => {
    ...
    runtime.begin('a', 1); now = 2; runtime.begin('b', 1); now = 3; runtime.read('b'); now = 4; runtime.begin('c', 1);
  ...
  it('preserves pinned windows during global eviction', () => {
    let now = 1;
    const runtime = createViewportResultWindowRuntime({ maximumWindows: 2 }, () => now);
    runtime.begin('a', 1, { pinned: true }); now = 2; runtime.begin('b', 1); now = 3; runtime.begin('c', 1);
  ...
  it('rolls back a page commit when global feature capacity cannot be satisfied', () => {
    const runtime = createViewportResultWindowRuntime({ maximumWindows: 2, maximumFeaturesPerWindow: 2, maximumTotalFeatures: 2 });
    runtime.commitPage({ windowKey: 'pinned', generation: 1, pageIndex: 0, features: [{ id: 1, payload: 'pinned' }] });
    runtime.pin('pinned', true);
    runtime.begin('current', 1, { pinned: true });
    expect(() => runtime.commitPage({ windowKey: 'current', generation: 1, pageIndex: 0, features: [{ id: 2, payload: 'a' }, { id: 3, payload: 'b' }] })).toThrow('global result-window budget');
  ...
  it('does not age out pinned windows', () => {
    let now = 0;
    const runtime = createViewportResultWindowRuntime({ maximumWindowAgeMs: 100 }, () => now);
    runtime.begin('active', 1, { pinned: true }); now = 1000;
    expect(runtime.snapshot().windowsSnapshot[0]?.key).toBe('active');
  });

  it('supports explicit unpinning before later eviction', () => {
    const runtime = createViewportResultWindowRuntime(); runtime.begin('active', 1, { pinned: true });
  ...
  it('marks windows complete without discarding earlier pages', () => {
    const runtime = createViewportResultWindowRuntime();
    runtime.commitPage({ windowKey: 'roads', generation: 1, pageIndex: 0, features: [] });
    const commit = runtime.commitPage({ windowKey: 'roads', generation: 1, pageIndex: 1, features: [], complete: true });
  ...
  it('drops only the requested generation', () => {
    const runtime = createViewportResultWindowRuntime(); runtime.begin('roads', 2);
    expect(runtime.drop('roads', 1)).toBe(false); expect(runtime.drop('roads', 2)).toBe(true); expect(runtime.snapshot().windows).toBe(0);
  });

  it('reports cleared window count', () => {
    const runtime = createViewportResultWindowRuntime(); runtime.begin('a', 1); runtime.begin('b', 1);
    expect(runtime.clear()).toBe(2); expect(runtime.clear()).toBe(0);
  });

  it('estimates payload bytes when callers do not supply estimates', () => {
    const runtime = createViewportResultWindowRuntime();
    runtime.commitPage({ windowKey: 'roads', generation: 1, pageIndex: 0, features: [{ id: 1, payload: { name: 'road' } }] });
    expect(runtime.snapshot().totalEstimatedBytes).toBeGreaterThan(0);
  });

  it('rejects malformed feature identities and byte estimates', () => {
    const runtime = createViewportResultWindowRuntime();
    expect(() => runtime.commitPage({ windowKey: 'roads', generation: 1, pageIndex: 0, features: [{ id: Number.NaN, payload: null }] })).toThrow('feature id');
    expect(() => runtime.commitPage({ windowKey: 'roads', generation: 1, pageIndex: 0, features: [{ id: 1, payload: null, estimatedBytes: -1 }] })).toThrow('estimatedBytes');
  });
  ...
  it('rejects malformed configuration eagerly', () => {
    ...
    expect(() => createViewportResultWindowRuntime({ maximumFeaturesPerWindow: 20, maximumTotalFeatures: 10 })).toThrow('maximumFeaturesPerWindow');
    expect(() => createViewportResultWindowRuntime({ maximumBytesPerWindow: 20, maximumTotalBytes: 10 })).toThrow('maximumBytesPerWindow');
  });

  it('freezes snapshots and read feature wrappers', () => {
    const runtime = createViewportResultWindowRuntime();
    runtime.commitPage({ windowKey: 'roads', generation: 1, pageIndex: 0, features: [{ id: 1, payload: 'a' }] });
    const snapshot = runtime.snapshot(); const values = runtime.read('roads');
    expect(Object.isFrozen(snapshot)).toBe(true); expect(Object.isFrozen(snapshot.windowsSnapshot)).toBe(true); expect(Object.isFrozen(snapshot.windowsSnapshot[0])).toBe(true); expect(Object.isFrozen(values)).toBe(true); expect(Object.isFrozen(values[0])).toBe(true);
  });

  it('rejects use after deterministic disposal', () => {
    const runtime = createViewportResultWindowRuntime(); runtime.dispose();
    expect(() => runtime.snapshot()).toThrow('disposed'); expect(() => runtime.begin('roads', 1)).toThrow('disposed');
  });
