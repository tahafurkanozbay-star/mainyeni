...
export const focusPickedGraphic = async (view: SceneViewLike, graphic: ArcGisGraphicLike, options: FocusGraphicOptions = {}): Promise<boolean> => { if (!graphic?.geometry || !view?.goTo || options.signal?.aborted) return false; const target = options.targetFactory ? options.targetFactory(graphic) : graphic.geometry; const goToOptions = { duration: Number.isFinite(options.duration) ? options.duration : 500, animate: options.animate !== false, ...(options.signal ? { signal: options.signal } : {}) }; try { await view.goTo(target, goToOptions); return !options.signal?.aborted; } catch (error) { if (options.signal?.aborted || (error as any)?.name === 'AbortError') return false; throw error; } };
...
export const applyViewStateToSceneView = async (view: SceneViewLike, inputState: ViewStateInput | ViewState = {}, options: SceneApplyOptions = {}): Promise<boolean> => {
  ...
  if (options.signal?.aborted) return false; const goToOptions = { duration: Math.max(0, finite(options.duration, 0) as number), animate: options.animate === true, ...(options.signal ? { signal: options.signal } : {}) };
  try { await view.goTo(target, goToOptions); return !options.signal?.aborted; } catch (error) { if (options.signal?.aborted || (error as any)?.name === 'AbortError') return false; throw error; }
};
...
export const applySceneBookmark = async (view: SceneViewLike, bookmark: SceneBookmark, options: { duration?: number; animate?: boolean; signal?: AbortSignal } = {}): Promise<boolean> => { if (!view?.goTo || !bookmark?.camera || options.signal?.aborted) return false; const goToOptions = { duration: Number.isFinite(options.duration) ? options.duration : 600, animate: options.animate !== false, ...(options.signal ? { signal: options.signal } : {}) }; try { await view.goTo({ camera: bookmark.camera, ...(Number.isFinite(bookmark.scale) ? { scale: bookmark.scale } : {}) }, goToOptions); return !options.signal?.aborted; } catch (error) { if (options.signal?.aborted || (error as any)?.name === 'AbortError') return false; throw error; } };
