...
export class QueryLifecycleError extends Error {
  public constructor(
    public readonly code: "invalid-key" | "queue-full" | "cancelled" | "timeout" | "disposed",
    message: string,
  ) {
    super(message);
...
const normalizePolicy = (input?: Partial<QueryLifecyclePolicy>): QueryLifecyclePolicy => {
  ...
  return Object.freeze({
    ...
    dedupeTtlMs:
      Number.isSafeInteger(merged.dedupeTtlMs) && merged.dedupeTtlMs >= 0
        ? merged.dedupeTtlMs
        : DEFAULT_POLICY.dedupeTtlMs,
  });
};
...
export class QueryLifecycleCoordinator {
  ...
  public constructor(policy?: Partial<QueryLifecyclePolicy>) {
    this.#policy = normalizePolicy(policy);
  }

  public execute<T>(request: QueryRequest<T>): Promise<T> {
    if (this.#disposed) {
      return Promise.reject(new QueryLifecycleError("disposed", "Query lifecycle coordinator is disposed."));
    }

    const key = normalizeKey(request.key);
    if (key.length === 0 || key.length > 512) {
      this.#rejected += 1;
      return Promise.reject(new QueryLifecycleError("invalid-key", "Query key must contain 1-512 characters."));
    }

    if (request.signal?.aborted) {
      this.#cancelled += 1;
      return Promise.reject(new QueryLifecycleError("cancelled", "Query subscriber was already cancelled."));
    }
    ...
    if (this.#queue.length >= this.#policy.maxQueued && this.#active >= this.#policy.maxConcurrent) {
      this.#rejected += 1;
      return Promise.reject(new QueryLifecycleError("queue-full", "Bounded GIS query queue is full."));
    }
    ...
    this.#queue.sort((left, right) =>
      PRIORITY_WEIGHT[left.priority] - PRIORITY_WEIGHT[right.priority] || left.sequence - right.sequence,
    );
    this.#drain();
    return promise;
  }
      ...
      if (signal) {
        subscriber.abortListener = () => {
          ...
          if (item.subscribers.size === 0) {
            item.controller.abort(new QueryLifecycleError("cancelled", "All GIS query subscribers cancelled."));
            if (!item.started) {
  ...
  #drain(): void {
    while (!this.#disposed && this.#active < this.#policy.maxConcurrent && this.#queue.length > 0) {
    ...
    try {
      const timeout = new Promise<never>((_, reject) => {
        timeoutHandle = setTimeout(() => {
          timedOut = true;
          const error = new QueryLifecycleError("timeout", `GIS query exceeded ${this.#policy.timeoutMs}ms.`);
          item.controller.abort(error);
          reject(error);
        }, this.#policy.timeoutMs);
      });
      const value = await Promise.race([item.execute(item.controller.signal), timeout]);
      if (this.#policy.dedupeTtlMs > 0) {
        this.#recent.set(item.key, { value, expiresAt: Date.now() + this.#policy.dedupeTtlMs });
      }
      this.#completed += 1;
      this.#settleSubscribers(item, "resolve", value);
    } catch (error) {
      if (timedOut) this.#timedOut += 1;
      else if (item.controller.signal.aborted) this.#cancelled += 1;
      else this.#failed += 1;
      this.#settleSubscribers(item, "reject", error);
    } finally {
