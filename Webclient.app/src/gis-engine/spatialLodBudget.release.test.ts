...
import { DEFAULT_SPATIAL_LOD_POLICY, decideSpatialLod, lodClusterTarget, lodRequestBudget, type LodPressure, type SpatialLodDecision, type SpatialLodPolicy } from "./spatialLodBudget";

const pressureRank: Record<LodPressure, number> = { normal: 0, warm: 1, hot: 2, critical: 3 };
const policy: SpatialLodPolicy = { ...DEFAULT_SPATIAL_LOD_POLICY, minLevel: 2, maxLevel: 18, maxVisibleFeatures: 1_000, maxEstimatedVertices: 10_000, warmFeatureRatio: 0.5, hotFeatureRatio: 0.75, criticalFeatureRatio: 1 };
const decide = (visibleFeatures: number, estimatedVertices: number, framePressure: LodPressure = "normal", requestedLevel = 12): SpatialLodDecision => {
  const decision = decideSpatialLod({ requestedLevel, visibleFeatures, estimatedVertices, framePressure }, policy);
...
describe("spatial LOD release invariants", () => {
  ...
  it("never increases effective detail as pressure rises", () => {
    let featureLevel = Number.POSITIVE_INFINITY;
    let vertexLevel = Number.POSITIVE_INFINITY;
    for (let step = 0; step <= 60; step += 1) {
      const featureCurrent = decide(step * 25, 0).effectiveLevel;
      const vertexCurrent = decide(0, step * 250).effectiveLevel;
      expect(featureCurrent).toBeLessThanOrEqual(featureLevel);
      expect(vertexCurrent).toBeLessThanOrEqual(vertexLevel);
      featureLevel = featureCurrent;
      vertexLevel = vertexCurrent;
    }
  });
  it("preserves exact feature thresholds", () => {
  ...
  it("preserves exact vertex thresholds", () => {
  ...
  it("reports frame provenance only when data pressure is normal", () => {
    for (const framePressure of ["warm", "hot", "critical"] as const) {
      expect(decide(100, 1_000, framePressure)).toMatchObject({ pressure: framePressure, reason: "frame-pressure" });
    }
  });
  it("uses strongest source without losing combined provenance", () => {
  ...
  it("never degrades below configured minimum", () => {
    for (const requestedLevel of [2, 3, 4, 5]) expect(decide(10_000, 100_000, "critical", requestedLevel).effectiveLevel).toBeGreaterThanOrEqual(2);
  });
  it("keeps degraded consistent with effective level", () => {
  ...
  it("scales cluster and request budgets monotonically", () => {
    const pressures = ["normal", "warm", "hot", "critical"] as const;
    expect(pressures.map(frame => lodClusterTarget(decide(0, 0, frame), 100))).toEqual([100, 200, 300, 400]);
    expect(pressures.map(frame => lodRequestBudget(decide(0, 0, frame), 12))).toEqual([12, 6, 4, 3]);
  });
  it("keeps request concurrency at least one under critical pressure", () => {
    for (const base of [1, 2, 3, 4]) expect(lodRequestBudget(decide(0, 0, "critical"), base)).toBe(1);
  });
  ...
  it("rejects malformed spatial observations", () => {
  ...
  it("fails closed for invalid policy ordering and capacity", () => {
    expect(() => decideSpatialLod({ requestedLevel: 4, visibleFeatures: 0, estimatedVertices: 0 }, { warmFeatureRatio: 0.9, hotFeatureRatio: 0.8 })).toThrow(/hotFeatureRatio/);
    expect(() => decideSpatialLod({ requestedLevel: 4, visibleFeatures: 0, estimatedVertices: 0 }, { hotFeatureRatio: 0.95, criticalFeatureRatio: 0.9 })).toThrow(/criticalFeatureRatio/);
