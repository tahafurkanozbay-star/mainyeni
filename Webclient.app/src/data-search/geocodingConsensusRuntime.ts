...
export const GEOCODING_CONSENSUS_VERSION = '2026-09-24.v2';
...
const providerId = (value: unknown): string => normalizeSearchText(value).replace(/[^a-z0-9_-]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 80);
...
const normalizeOptions = (input: GeocodingConsensusOptions = {}): NormalizedOptions => {
  ...
  return Object.freeze({
    maxProviders,
    maxConcurrentProviders: normalizeInteger(input.maxConcurrentProviders, { min: 1, max: maxProviders, fallback: Math.min(4, maxProviders) }),
    ...
    minimumSuccessfulProviders: normalizeInteger(input.minimumSuccessfulProviders, { min: 1, max: maxProviders, fallback: 1 }),
    minimumAgreementProviders: normalizeInteger(input.minimumAgreementProviders, { min: 1, max: maxProviders, fallback: 1 }),
...
const selectProviders = (
  ...
  const requested = execution.providerIds ? new Set(execution.providerIds.map(providerId).filter(Boolean)) : null;
  ...
  eligible.sort((left, right) => (Number(right.priority) || 0) - (Number(left.priority) || 0) || left.id.localeCompare(right.id, 'en'));
  return Object.freeze(eligible.slice(0, options.maxProviders));
};

const createRequestFingerprint = (operation: GeocodingOperation, request: Request, providers: readonly GeocodingConsensusProvider[]): string =>
  hashFingerprint(stableSerialize({ version: GEOCODING_CONSENSUS_VERSION, operation, request, providerIds: providers.map(provider => provider.id) }));

const adapt = (payload: unknown, request: Request, options: NormalizedOptions): GeocodePage => adaptGeocodingPayload(payload, {
...
const executeProvider = async (
  ...
  const context: GeocodingProviderContext = Object.freeze({ signal: controller.signal, requestId, providerId: provider.id, operation });
  try {
    ...
    const timeoutMs = normalizeInteger(provider.timeoutMs, { min: 100, max: 120_000, fallback: options.providerTimeoutMs });
    ...
    const abort = new Promise<never>((_resolve, reject) => {
      controller.signal.addEventListener('abort', () => reject(createAbortError('Geocoding consensus provider aborted')), { once: true });
    });
    ...
    return page.candidates.length === 0
      ...
      : Object.freeze({ ok: true, providerId: provider.id, providerWeight: providerWeight(provider.weight), page });
  } catch {
    if (timedOut) return Object.freeze({ ok: false, providerId: provider.id, kind: 'timeout' });
    if (controller.signal.aborted || externalSignal?.aborted) return Object.freeze({ ok: false, providerId: provider.id, kind: 'aborted' });
...
const runBounded = async (
  ...
  const workers = Array.from({ length: workerCount }, (_, worker) => (async (): Promise<void> => {
    while (true) {
      ...
      if (!provider) return;
      results[index] = await executeProvider(provider, operation, request, `geocode-consensus-${fingerprint}-${worker}-${index}`, execution.signal, options);
    }
  })());
...
const betterEvidence = (left: GeocodingConsensusEvidence, right: GeocodingConsensusEvidence): GeocodingConsensusEvidence => {
...
const addEvidence = (
  ...
  if (state.evidenceItems >= options.maxEvidenceItems) return { ...state, inputCandidates, evidenceTruncated: true };
  const key = groupKey(candidate);
  const existing = state.groups.get(key);
  const group = existing ?? { key, canonicalLabel: canonicalizeAddressText(candidate.label), label: normalizeText(candidate.label), byProvider: new Map<string, GeocodingConsensusEvidence>() };
  const evidence: GeocodingConsensusEvidence = Object.freeze({ providerId: success.providerId, providerWeight: success.providerWeight, candidate });
...
const groupCandidates = (
  ...
  const initial: GroupingState = { groups: new Map<string, CandidateGroup>(), inputCandidates: 0, evidenceItems: 0, evidenceTruncated: false };
  ...
  return Object.freeze({ groups: Object.freeze([...state.groups.values()]), inputCandidates: state.inputCandidates, evidenceTruncated: state.evidenceTruncated });
};

const centroid = (evidence: readonly GeocodingConsensusEvidence[]): Coordinate | null => {
  const coordinates = evidence.map(item => item.candidate.coordinates).filter((value): value is Coordinate => value !== null);
  if (coordinates.length === 0) return null;
  const totals = coordinates.reduce((result, coordinate) => ({ latitude: result.latitude + coordinate.latitude, longitude: result.longitude + coordinate.longitude }), { latitude: 0, longitude: 0 });
  return Object.freeze({ latitude: totals.latitude / coordinates.length, longitude: totals.longitude / coordinates.length });
};

const coordinateSpread = (evidence: readonly GeocodingConsensusEvidence[], center: Coordinate | null): number => {
  if (!center) return 0;
...
const representative = (evidence: readonly GeocodingConsensusEvidence[]): GeocodingConsensusEvidence => {
  const first = [...evidence].sort((left, right) => right.candidate.score * right.providerWeight - left.candidate.score * left.providerWeight || right.candidate.score - left.candidate.score || left.providerId.localeCompare(right.providerId, 'en'))[0];
  if (!first) throw new Error('Geocoding consensus candidate requires evidence');
  return first;
};

const freezeCandidate = (group: CandidateGroup, successfulProviders: number, options: NormalizedOptions): GeocodingConsensusCandidate => {
  const evidence = Object.freeze([...group.byProvider.values()].sort((left, right) => left.providerId.localeCompare(right.providerId, 'en')));
  ...
  const weightedProviderScore = totalWeight > 0 ? Math.round(evidence.reduce((sum, item) => sum + clamp(item.candidate.score) * item.providerWeight, 0) / totalWeight) : 0;
  ...
  const coordinateScore = coordinates === null ? 50 : coordinateConflict ? Math.max(0, Math.round(100 * (1 - coordinateSpreadMeters / Math.max(1, options.coordinateToleranceMeters * 4)))) : Math.max(60, Math.round(100 * (1 - coordinateSpreadMeters / Math.max(1, options.coordinateToleranceMeters))));
  const confidence = clamp(Math.round(weightedProviderScore * 0.55 + agreementScore * 0.30 + coordinateScore * 0.15));
  const fingerprint = hashFingerprint(stableSerialize({ key: group.key, providerIds, weightedProviderScore, agreementScore, coordinates, coordinateSpreadMeters, confidence }));
  return Object.freeze({ key: group.key, label: selected.candidate.label || group.label, canonicalLabel: group.canonicalLabel, representative: selected.candidate, providerIds, providerCount: providerIds.length, agreementCount, weightedProviderScore, agreementScore, coordinateScore, confidence, coordinates, coordinateSpreadMeters, coordinateConflict, evidence, fingerprint });
};

const diagnostics = (providers: readonly GeocodingConsensusProvider[], results: readonly ProviderResult[], inputCandidates: number, outputCandidates: number, evidenceTruncated: boolean, candidateTruncated: boolean): GeocodingConsensusDiagnostics => {
  const failures = Object.freeze(results.filter((result): result is ProviderFailure => !result.ok).map(result => Object.freeze({ providerId: result.providerId, kind: result.kind })));
  return Object.freeze({ requestedProviders: providers.length, eligibleProviders: providers.length, successfulProviders: results.filter(result => result.ok).length, failedProviders: failures.length, timedOutProviders: failures.filter(item => item.kind === 'timeout').length, abortedProviders: failures.filter(item => item.kind === 'aborted').length, inputCandidates, outputCandidates, evidenceTruncated, candidateTruncated, failures });
};
...
export const executeGeocodingConsensus = async (
  ...
  if (successes.length < options.minimumSuccessfulProviders) throw new Error(`Geocoding consensus requires ${options.minimumSuccessfulProviders} successful provider(s); received ${successes.length}.`);
  const grouped = groupCandidates(successes, options);
  let candidates = grouped.groups.map(group => freezeCandidate(group, successes.length, options)).filter(candidate => candidate.agreementCount >= options.minimumAgreementProviders).sort((left, right) => right.confidence - left.confidence || right.agreementCount - left.agreementCount || right.weightedProviderScore - left.weightedProviderScore || left.label.localeCompare(right.label, 'tr-TR', { sensitivity: 'base', numeric: true }) || left.key.localeCompare(right.key, 'en'));
  ...
  const resultDiagnostics = diagnostics(providers, results, grouped.inputCandidates, frozenCandidates.length, grouped.evidenceTruncated, candidateTruncated);
  const fingerprint = hashFingerprint(stableSerialize({ version: GEOCODING_CONSENSUS_VERSION, operation, requestFingerprint, candidates: frozenCandidates.map(candidate => ({ fingerprint: candidate.fingerprint, confidence: candidate.confidence, providerIds: candidate.providerIds })), diagnostics: resultDiagnostics }));
  return Object.freeze({ version: GEOCODING_CONSENSUS_VERSION, operation, requestFingerprint, candidates: frozenCandidates, diagnostics: resultDiagnostics, partial, fingerprint });
};

export const executeForwardGeocodingConsensus = (providers: readonly GeocodingConsensusProvider[], request: ForwardGeocodeRequest, options: GeocodingConsensusOptions = {}, execution: GeocodingConsensusExecutionOptions = {}): Promise<GeocodingConsensusResult> => executeGeocodingConsensus(providers, 'forward', request, options, execution);
export const executeReverseGeocodingConsensus = (providers: readonly GeocodingConsensusProvider[], request: ReverseGeocodeRequest, options: GeocodingConsensusOptions = {}, execution: GeocodingConsensusExecutionOptions = {}): Promise<GeocodingConsensusResult> => executeGeocodingConsensus(providers, 'reverse', request, options, execution);
