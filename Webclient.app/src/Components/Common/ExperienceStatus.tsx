...
export function ExperienceStatus({ children, tone = 'neutral', live = 'off', busy = false, className = '' }: ExperienceStatusProps): ReactNode {
  ...
  return (
    <output className={`experience-status experience-status--${tone} ${className}`.trim()} aria-busy={busy || undefined} data-tone={tone} {...liveProps}>
...
export function ExperienceProgress({ value, max = 100, label, detail }: ExperienceProgressProps): ReactNode {
  ...
  return (
    <div className="experience-progress" aria-label={label}>
      <div className="experience-progress__meta"><span>{label}</span>{detail ? <span>{detail}</span> : null}</div>
      {determinate ? <progress value={boundedValue} max={boundedMax}>{Math.round((boundedValue! / boundedMax) * 100)}%</progress> : (
        <div className="experience-progress__indeterminate" role="progressbar" aria-label={label} aria-valuetext="İşlem sürüyor"><span aria-hidden="true" /></div>
      )}
    </div>
  );
}
