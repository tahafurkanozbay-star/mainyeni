...
export function ExperienceUXLayer({ windowManager }) {
    ...
    useEffect(() => {
        const onCommand = (event) => {
            ...
            if (name === "layers" && windowManager?.ShowWindow) windowManager.ShowWindow("layerlist-widget");
            if (name === "legend" && windowManager?.ShowWindow) windowManager.ShowWindow("layerlist-widget");
    ...
    useEffect(() => {
        if (helpOpen) requestAnimationFrame(() => closeButtonRef.current?.focus());
    }, [helpOpen]);
        ...
        <>
            <aside className={`experience-utility ${collapsed ? "is-collapsed" : ""}`} aria-label="Kent Rehberi yardımcı araçları">
                ...
                <nav className="experience-utility__actions" aria-label="Hızlı araçlar">
                    <button className="experience-utility__button" type="button" onClick={() => dispatchCommand("search")} aria-label="Genel aramayı aç" title="Genel arama">
                        <ActionIcon type="search" />{!collapsed && <span>Arama</span>}
                    </button>
                    <button className="experience-utility__button" type="button" onClick={() => dispatchCommand("layers")} aria-label="Katman yönetimini aç" title="Katmanlar">
                    ...
                    <button className="experience-utility__button" type="button" onClick={() => setHelpOpen(true)} aria-label="Kısayolları ve yardım bilgisini aç" title="Kısayollar">
                        <ActionIcon type="keyboard" />{!collapsed && <span>Kısayollar</span>}
                    </button>
                    <button className="experience-utility__button" type="button" onClick={toggleTheme} aria-label={themeLabel} title={themeLabel}>
                    ...
                    <section className="experience-help" role="dialog" aria-modal="true" aria-labelledby="experience-help-title" aria-describedby="experience-help-description" onMouseDown={event => event.stopPropagation()}>
                        <header className="experience-help__header">
                            <div>
                                ...
                                <p id="experience-help-description">Harita ve veri araçlarına klavye ile hızlı erişin.</p>
                            </div>
                            <button ref={closeButtonRef} className="experience-close" type="button" onClick={() => setHelpOpen(false)} aria-label="Yardım penceresini kapat">×</button>
                        </header>
                        ...
                        <footer className="experience-help__footer">
                            ...
                            <span>Mevcut GIS sorgu, katman ve popup işlevleri korunur; bu yüzey yalnızca keşif, erişilebilirlik ve hızlı erişim sağlar.</span>
                        </footer>
                    </section>
                </div>
            )}
