...
export function ExperiencePagination({ page, totalPages, onPageChange, label = 'Sayfalama', siblingCount = 1, disabled = false }: ExperiencePaginationProps): ReactNode {
  ...
  return (
    <nav className="experience-pagination" aria-label={label}>
      <button type="button" className="experience-pagination__button" onClick={() => changePage(safePage - 1)} disabled={disabled || safePage <= 1} aria-label="Önceki sayfa">‹</button>
      <ol className="experience-pagination__pages">
        {items.map((item) => typeof item === 'number' ? (
          <li key={item}>
            <button type="button" className="experience-pagination__button" onClick={() => changePage(item)} disabled={disabled} aria-current={item === safePage ? 'page' : undefined} aria-label={`${item}. sayfa`}>{item}</button>
          </li>
        ) : <li key={item} className="experience-pagination__ellipsis" aria-hidden="true">…</li>)}
      </ol>
      <button type="button" className="experience-pagination__button" onClick={() => changePage(safePage + 1)} disabled={disabled || safePage >= safeTotal} aria-label="Sonraki sayfa">›</button>
    </nav>
  );
}
