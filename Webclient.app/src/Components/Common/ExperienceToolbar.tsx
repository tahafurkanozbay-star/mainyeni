  ...
  return (
    <div className={`experience-toolbar experience-toolbar--${orientation} ${className}`.trim()} role="toolbar" aria-label={label} aria-orientation={orientation}>
      {actions.map((action, index) => (
        <button key={action.id} id={`${id}-${action.id}`} type="button" className="experience-toolbar__action" aria-label={action.label} aria-pressed={action.pressed} disabled={action.disabled} tabIndex={index === firstEnabledIndex ? 0 : -1} onClick={action.onActivate} onKeyDown={(event) => handleToolbarKeyDown(event, orientation)}>
