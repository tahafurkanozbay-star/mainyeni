...
export const EgoKartSatisNoktalariQueryWindow = React.forwardRef((props, ref) => {

    const windowTitle = "Ego Kart Satış Noktaları";
    const windowLogo = "images/Sidebar/EGO/egokartsatis.png";
    const windowLogoicon = "images/icons/map/EGO/kartsatis.svg";
   ...
   const fetchQueryResults = () => {
        ...
        LoggingBusiness.CreateClientLog("Ego Kart/Sorgu", query.districtName+"/"+query.nbhoodName+"/"+query.name);

        EgoKartSatisNoktalariQueryBusiness.Query(query,false).then((_result) => {
            ...
            if (_result.type == Constants_ServiceResultType.Success) {
                ...
                mapView.goTo(targetExtent).then(() => {
                    ...
                    CommonBusiness.Clustering.CreateLayerWithoutClustering("YeniEgoKartSatisNoktalariQueryUrl", props.windowTitle, query, initialSymbol).then((_clusterLayer) => {
                        removeLastClusterLayer();
    ...
    const getItemDetailsById=async(_item)=>{
        ...
        return new Promise((resolve, reject)=>{
            EgoKartSatisNoktalariQueryBusiness.Query({ObjectId: _item.ObjectId},true).then((_result) => {
                if(_result.type==Constants_ServiceResultType.Success){
