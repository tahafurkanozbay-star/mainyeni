...
export const HalkEkmekQueryWindow = React.forwardRef((props, ref) => {

    const windowTitle = "Halk Ekmek";
    const windowLogo = "images/icons/sidebar/halkekmek.png";
    const windowLogoicon = "images/icons/map/ISTIRAK/halkekmek.svg";
   ...
   const fetchQueryResults = () => {
        ...
        LoggingBusiness.CreateClientLog("Halk Ekmek/Sorgu", query.districtName+"/"+query.nbhoodName+"/"+query.name);

        HalkEkmekQueryBusiness.Query(query,false).then((_result) => {
            ...
            if (_result.type == Constants_ServiceResultType.Success) {
                ...
                mapView.goTo(targetExtent).then(() => {
                    ...
                    CommonBusiness.Clustering.CreateLayerWithoutClustering("YeniHalkEkmekSatisNoktalariQeryUrl", props.windowTitle, query, initialSymbol).then((_clusterLayer) => {
                        removeLastClusterLayer();
    ...
    const getItemDetailsById=async(_item)=>{
        ...
        return new Promise((resolve, reject)=>{
            HalkEkmekQueryBusiness.Query({ObjectId: _item.ObjectId},true).then((_result) => {
                if(_result.type==Constants_ServiceResultType.Success){
