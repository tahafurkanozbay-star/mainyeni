...
const DEFAULT_FOCUS_SELECTOR = ['[autofocus]','input:not([disabled])','select:not([disabled])','textarea:not([disabled])','button:not([disabled])','a[href]','[tabindex]:not([tabindex="-1"])'].join(',');
...
const isConnectedFocusable = (element: HTMLElement | null | undefined): element is HTMLElement => Boolean(element && !isUnavailable(element));
...
const findInitialFocusTarget = (root: HTMLElement, selector: string): HTMLElement | null => {
  try {
    ...
    for (const candidate of candidates) if (isConnectedFocusable(candidate)) return candidate;
  } catch { return null; }
  return null;
};
...
export function focusManagedWindow({ root, initialFocusSelector = DEFAULT_FOCUS_SELECTOR }: ManagedWindowFocusOptions): HTMLElement | null {
  ...
  if (target) { target.focus({ preventScroll: true }); return target; }
...
const shouldRestoreFocus = (documentRef: Document, root: HTMLElement | null, policy: ManagedWindowFocusRestorePolicy): boolean => {
...
export function createManagedWindowFocusLifecycle(documentRef: Document = document): ManagedWindowFocusLifecycle {
  ...
  const cancelPending = () => { cancelScheduledFocus?.(); cancelScheduledFocus = null; };
  const resetCycle = () => { opener = null; root = null; restorePolicy = 'if-focus-within'; };
  return {
    isOpen: () => opened,
    open: options => {
      cancelPending();
      if (!opened) {
        opener = Object.prototype.hasOwnProperty.call(options, 'opener') ? options.opener ?? null : captureManagedWindowOpener(documentRef);
    ...
    close: () => {
      ...
      opened = false;
      cancelPending();
      const restored = shouldRestoreFocus(documentRef, root, restorePolicy) ? restoreManagedWindowFocus(opener) : false;
      resetCycle();
      return restored;
    },
    dispose: () => { opened = false; cancelPending(); resetCycle(); }
  };
}
