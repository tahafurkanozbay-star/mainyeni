...
export const WifiNoktalariQueryWindow = React.forwardRef((props, ref) => {

    const windowTitle = "Wifi Noktaları";
    const windowLogo = "images/Sidebar/ABB/wifi.png";
    const windowLogoicon = "images/icons/map/ABB/wifierisimnoktalari.svg";
    ...
    const fetchQueryResults = () => {
        ...
        LoggingBusiness.CreateClientLog("wifi/Sorgu", query.districtName+"/"+query.nbhoodName+"/"+query.name);

        WifiNoktalariQeryBusiness.Query(query,false).then((_result) => {
            ...
            if (_result.type == Constants_ServiceResultType.Success) {
                ...
                mapView.goTo(targetExtent).then(() => {
                    ...
                    CommonBusiness.Clustering.CreateLayerWithoutClustering("YeniWifiNoktalariQeryUrl", props.windowTitle, query, initialSymbol).then((_clusterLayer) => {
                        removeLastClusterLayer();
    ...
    const getItemDetailsById=async(_item)=>{
        ...
        return new Promise((resolve, reject)=>{
            WifiNoktalariQeryBusiness.Query({ObjectId: _item.ObjectId},true).then((_result) => {
                if(_result.type==Constants_ServiceResultType.Success){
