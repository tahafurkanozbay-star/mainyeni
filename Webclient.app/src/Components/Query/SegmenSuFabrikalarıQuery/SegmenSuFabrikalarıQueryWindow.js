...
export const SegmenSuFabrikalarıQueryWindow = React.forwardRef((props, ref) => {

    const windowTitle = "Seğmen Su Fabrikaları";
    const windowLogo = "images/Sidebar/ISTIRAK/segmensufabrikalari.png";
    const windowLogoicon = "images/icons/map/ISTIRAK/segmensu.svg";
   ...
   const fetchQueryResults = () => {
        ...
        LoggingBusiness.CreateClientLog("segmen su/Sorgu", query.districtName+"/"+query.nbhoodName+"/"+query.name);

        SegmenSuFabrikalarıQeryBusiness.Query(query,false).then((_result) => {
            ...
            if (_result.type == Constants_ServiceResultType.Success) {
                ...
                mapView.goTo(targetExtent).then(() => {
                    ...
                    CommonBusiness.Clustering.CreateLayerWithoutClustering("YeniSegmenSuFabrikalarıQeryUrl", props.windowTitle, query, initialSymbol).then((_clusterLayer) => {
                        removeLastClusterLayer();
    ...
    const getItemDetailsById=async(_item)=>{
        ...
        return new Promise((resolve, reject)=>{
            SegmenSuFabrikalarıQeryBusiness.Query({ObjectId: _item.ObjectId},true).then((_result) => {
                if(_result.type==Constants_ServiceResultType.Success){
