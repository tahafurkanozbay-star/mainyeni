...
export const PatiDostuQueryWindow = React.forwardRef((props, ref) => {

    const windowTitle = "Pati Dostu";
    const windowLogo = "images/Sidebar/ABB/patidostu.png";
    const windowLogoicon = "images/icons/map/ABB/patidostuuygulamalar.svg";
   ...
   const fetchQueryResults = () => {
        ...
        LoggingBusiness.CreateClientLog("Pati Dostu/Sorgu", query.districtName+"/"+query.nbhoodName+"/"+query.name);

        PatiDostuQueryBusiness.Query(query,false).then((_result) => {
            ...
            if (_result.type == Constants_ServiceResultType.Success) {
                ...
                mapView.goTo(targetExtent).then(() => {
                    ...
                    CommonBusiness.Clustering.CreateLayerWithoutClustering("YeniPatiDostuQeryUrl", props.windowTitle, query, initialSymbol).then((_clusterLayer) => {
                        removeLastClusterLayer();
    ...
    const getItemDetailsById=async(_item)=>{
        ...
        return new Promise((resolve, reject)=>{
            PatiDostuQueryBusiness.Query({ObjectId: _item.ObjectId},true).then((_result) => {
                if(_result.type==Constants_ServiceResultType.Success){
