...
export const BelmekBeltekQeryWindow = React.forwardRef((props, ref) => {

    const windowTitle = "Belmek-Beltek";
    const windowLogo = "images/Sidebar/ABB/belmek.png";
    const windowLogoicon = "images/icons/map/ABB/belmek.svg";
   ...
   const fetchQueryResults = () => {
        ...
        LoggingBusiness.CreateClientLog("Belmek/Sorgu", query.districtName+"/"+query.nbhoodName+"/"+query.name);

        BelmekBeltekQeryBusiness.Query(query,false).then((_result) => {
            ...
            if (_result.type == Constants_ServiceResultType.Success) {
                ...
                mapView.goTo(targetExtent).then(() => {
                    ...
                    CommonBusiness.Clustering.CreateLayerWithoutClustering("YeniBelmekBeltekQeryUrl", props.windowTitle, query, initialSymbol).then((_clusterLayer) => {
                        removeLastClusterLayer();
                        ...
                        _clusterLayer.layerObj.queryExtent().then((response) => {
                            if (response.extent) {
                                ...
                                let expandFactorY = (extent.ymax - extent.ymin) * 0.05;  // Y ekseninde %10 genişlet
    ...
    const getItemDetailsById=async(_item)=>{
        ...
        return new Promise((resolve, reject)=>{
            BelmekBeltekQeryBusiness.Query({ObjectId: _item.ObjectId},true).then((_result) => {
                if(_result.type==Constants_ServiceResultType.Success){
