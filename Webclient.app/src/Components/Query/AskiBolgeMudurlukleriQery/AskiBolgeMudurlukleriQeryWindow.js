...
export const AskiBolgeMudurlukleriQeryWindow = React.forwardRef((props, ref) => {

    const windowTitle = "Aski Bölge Müdürlükleri";
    const windowLogo = "images/Sidebar/ASKI/askibolgemudurlukleri.png";
    const windowLogoicon = "images/icons/map/ASKİ/bolgemudurlukleri.svg";
   ...
   const fetchQueryResults = () => {
        ...
        LoggingBusiness.CreateClientLog("BÖLGE MÜD. su/Sorgu", query.districtName+"/"+query.nbhoodName+"/"+query.name);

        AskiBolgeMudurlukleriQeryBusiness.Query(query,false).then((_result) => {
            ...
            if (_result.type == Constants_ServiceResultType.Success) {
                ...
                mapView.goTo(targetExtent).then(() => {
                    ...
                    CommonBusiness.Clustering.CreateLayerWithoutClustering("YeniAskiBolgeMudurlukleriQeryUrl", props.windowTitle, query, initialSymbol).then((_clusterLayer) => {
                        removeLastClusterLayer();
    ...
    const getItemDetailsById=async(_item)=>{
        ...
        return new Promise((resolve, reject)=>{
            AskiBolgeMudurlukleriQeryBusiness.Query({ObjectId: _item.ObjectId},true).then((_result) => {
                if(_result.type==Constants_ServiceResultType.Success){
