...
export const EgoOtobusDuraklariQueryWindow = React.forwardRef((props, ref) => {

    const windowTitle = "Ego Otobüs Durakları";
    const windowLogo = "images/Sidebar/EGO/otobusduraklari.png";
    const windowLogoicon = "images/icons/map/EGO/otobusduraklari.svg";
   ...
   const fetchQueryResults = () => {
        ...
        LoggingBusiness.CreateClientLog("otobüs/Sorgu", query.districtName+"/"+query.nbhoodName+"/"+query.name);

        EgoOtobusDuraklariQueryBusiness.Query(query,false).then((_result) => {
            ...
            if (_result.type == Constants_ServiceResultType.Success) {
                ...
                mapView.goTo(targetExtent).then(() => {
                    ...
                    CommonBusiness.Clustering.CreateLayerWithoutClustering("YeniEgoOtobusDuraklariQueryUrl", props.windowTitle, query, initialSymbol).then((_clusterLayer) => {
                        removeLastClusterLayer();
    ...
    const getItemDetailsById=async(_item)=>{
        ...
        return new Promise((resolve, reject)=>{
            EgoOtobusDuraklariQueryBusiness.Query({ObjectId: _item.ObjectId},true).then((_result) => {
                if(_result.type==Constants_ServiceResultType.Success){
