...
export const KutuphanelerQueryWindow = React.forwardRef((props, ref) => {

    const windowTitle = "Kütüphaneler";
    const windowLogo = "images/Sidebar/ABB/kutuphaneler.png";
    const windowLogoicon = "images/icons/map/ABB/kutuphane.svg";
   ...
   const fetchQueryResults = () => {
        ...
        LoggingBusiness.CreateClientLog("Kütüphane/Sorgu", query.districtName+"/"+query.nbhoodName+"/"+query.name);

        KutuphanelerQueryBusiness.Query(query,false).then((_result) => {
            ...
            if (_result.type == Constants_ServiceResultType.Success) {
                ...
                mapView.goTo(targetExtent).then(() => {
                    ...
                    CommonBusiness.Clustering.CreateLayerWithoutClustering("YeniKutuphanelerQueryUrl", props.windowTitle, query, initialSymbol).then((_clusterLayer) => {
                        removeLastClusterLayer();
    ...
    const getItemDetailsById=async(_item)=>{
        ...
        return new Promise((resolve, reject)=>{
            KutuphanelerQueryBusiness.Query({ObjectId: _item.ObjectId},true).then((_result) => {
                if(_result.type==Constants_ServiceResultType.Success){
