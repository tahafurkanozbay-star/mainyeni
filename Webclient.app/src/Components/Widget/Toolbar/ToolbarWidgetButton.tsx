...
export interface ToolbarWidgetButtonProps {
  readonly tooltipText: string;
  readonly onClick: MouseEventHandler<HTMLButtonElement>;
  readonly image?: string;
  readonly icon?: ToolbarGlyphName;
}

const ToolbarGlyph = ({
  name,
}: ToolbarGlyphProps): ReactNode => {
...
export const ToolbarWidgetButton = ({
  tooltipText,
  onClick,
  image,
  icon,
}: ToolbarWidgetButtonProps): ReactNode => {
  const tooltipId = `toolbar-tooltip-${String(image || tooltipText || 'control')
    .replace(/[^a-z0-9_-]/gi, '-')}`;
