...
export const normalizeAxiosError = (error) => {
  ...
  if (status === 408 || error?.code === 'ECONNABORTED') return new AppError('İstek zaman aşımına uğradı.', { code: 'TIMEOUT', status: status || 408, retryable: true, cause: error });
  ...
  return new AppError(serverMessage || 'İstek tamamlanamadı.', { code: 'HTTP_ERROR', status, retryable: false, cause: error });
};
