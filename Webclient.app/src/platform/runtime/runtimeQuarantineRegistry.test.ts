...
describe('RuntimeQuarantineRegistry', () => {
  ...
  it('releases probation immediately after a healthy result', () => {
    ...
    expect(registry.reportHealthy('identify', 14)).toBe(false);
  });
  ...
  it('refreshes quarantine without growing registry ownership', () => {
    ...
    expect(snapshot.entries[0]?.reason).toBe('failure-budget');
    expect(snapshot.entries[0]?.eligibleAt).toBe(52);
  });

  it('evicts deterministically at the configured capacity', () => {
  ...
  it('clears only the requested lane', () => {
    ...
    const snapshot = registry.clear('background');
    expect(snapshot.entries.map((entry) => entry.key)).toEqual(['critical-work']);
    expect(registry.has('background-work')).toBe(false);
  });
