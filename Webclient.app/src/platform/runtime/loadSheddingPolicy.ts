...
const ratio = (value: number, limit: number): number =>
  limit > 0 ? Math.max(0, value) / limit : Number.POSITIVE_INFINITY;
...
export class LoadSheddingPolicy {
  ...
  public constructor(thresholds: LoadSheddingThresholds) {
    if (
      thresholds.maxActive <= 0 ||
      thresholds.maxQueued < 0 ||
      thresholds.maxLatencyMs <= 0 ||
  ...
  public evaluate(sample: LoadSheddingSample, priority: LoadSheddingPriority): LoadSheddingDecision {
    const dimensions = {
      ...
      queue: this.#thresholds.maxQueued === 0
        ? (sample.queued > 0 ? Number.POSITIVE_INFINITY : 0)
        : ratio(sample.queued, this.#thresholds.maxQueued),
    ...
    if (pressure < 1) {
      return { admitted: true, pressure: clamp01(pressure), reason: 'healthy' };
    }

    const dominant = Object.entries(dimensions).reduce((best, current) =>
      current[1] > best[1] ? current : best,
    ) as [Exclude<LoadSheddingDecision['reason'], 'healthy' | 'priority'>, number];

    if (priority === 'critical' && pressure < 1.5) {
      return { admitted: true, pressure: clamp01(pressure), reason: 'priority' };
    }
    if (priority === 'interactive' && pressure < 1.15) {
      return { admitted: true, pressure: clamp01(pressure), reason: 'priority' };
    }
