...
describe('RuntimeFairShareAllocator', () => {
  ...
  it('enforces global queue capacity', () => {
    const allocator = new RuntimeFairShareAllocator({ capacity: 1, maximumRequestUnits: 1, maximumQueue: 1, maximumPerLaneQueue: 1 });
    allocator.request({ id: 'active', lane: 'critical', priority: 'urgent', units: 1, at: 1 });
    allocator.request({ id: 'q1', lane: 'interactive', priority: 'standard', units: 1, at: 2 });
    expect(allocator.request({ id: 'q2', lane: 'background', priority: 'standard', units: 1, at: 3 })).toMatchObject({ disposition: 'rejected', reason: 'queue-full' });
  });

  it('enforces per-lane queue capacity independently', () => {
    const allocator = new RuntimeFairShareAllocator({ capacity: 1, maximumRequestUnits: 1, maximumQueue: 3, maximumPerLaneQueue: 1 });
    allocator.request({ id: 'active', lane: 'critical', priority: 'urgent', units: 1, at: 1 });
  ...
  it('does not release unknown ownership', () => {
    const allocator = new RuntimeFairShareAllocator();
    expect(allocator.release('missing', 1)).toEqual([]);
  });
  ...
  it('keeps decision history bounded', () => {
    ...
    expect(allocator.history().map((event) => event.id)).toEqual(['b', 'b']);
  });

  it('can disable decision history', () => {
    const allocator = new RuntimeFairShareAllocator({ maximumHistory: 0 });
    allocator.request({ id: 'a', lane: 'critical', priority: 'urgent', units: 1, at: 1 });
    expect(allocator.history()).toEqual([]);
  });

  it('returns immutable snapshot structures', () => {
    const allocator = new RuntimeFairShareAllocator();
  ...
  it('returns immutable copies of active grants and history', () => {
    const allocator = new RuntimeFairShareAllocator();
    allocator.request({ id: 'a', lane: 'critical', priority: 'urgent', units: 1, at: 1 });
    ...
    expect(Object.isFrozen(allocator.history()[0])).toBe(true);
  });
  ...
  it('rejects unsafe numeric request values', () => {
    const allocator = new RuntimeFairShareAllocator();
    expect(() => allocator.request({ id: 'nan', lane: 'critical', priority: 'urgent', units: Number.NaN, at: 1 })).toThrow();
    expect(() => allocator.request({ id: 'fraction', lane: 'critical', priority: 'urgent', units: 1.5, at: 1 })).toThrow();
    expect(() => allocator.request({ id: 'unsafe', lane: 'critical', priority: 'urgent', units: 1, at: Number.MAX_VALUE })).toThrow();
  });

  it.each([
    { capacity: 0 },
    { maximumQueue: -1 },
    { maximumPerLaneQueue: -1 },
    { maximumHistory: -1 },
    { criticalWeight: 0 },
    { interactiveWeight: 0 },
    { backgroundWeight: 0 },
    { maximumRequestUnits: 0 },
    { capacity: 2, maximumRequestUnits: 3 },
    { maximumQueue: 1, maximumPerLaneQueue: 2 },
  ])('rejects invalid policy %#', (policy) => {
    expect(() => new RuntimeFairShareAllocator(policy)).toThrow();
  });

  it('resets all ownership, queues, history, deficits and clock state', () => {
    const allocator = new RuntimeFairShareAllocator({ capacity: 1, maximumRequestUnits: 1 });
  ...
  it('does not bypass an older large request within the same priority lane', () => {
    ...
    const promoted = allocator.release('holder', 4);
    expect(promoted[0]?.id).toBe('large');
  });
