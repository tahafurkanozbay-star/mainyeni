...
export interface PressureSummary {
  ...
  readonly byKind: Readonly<Record<PressureSignalKind, Readonly<{ count: number; average: number; peak: number }>>>;
...
const normalizedPolicy = (policy: PressureWindowPolicy) => Object.freeze({
  windowMs: positiveInteger(policy.windowMs, DEFAULT_POLICY.windowMs, 600_000),
...
const levelForScore = (score: number, policy: ReturnType<typeof normalizedPolicy>): PressureLevel => {
...
export const createPressureMonitor = (
  ...
  const calculate = (timestamp: number): PressureSummary => {
    ...
    if (LEVEL_RANK[observedLevel] > LEVEL_RANK[currentLevel]) {
      ...
      consecutiveRecoverySamples = score <= policy.recoveryThreshold ? consecutiveRecoverySamples + 1 : 0;
  ...
  const record = (sample: PressureSample): PressureSummary => {
    if (disposed || !SIGNAL_KINDS.includes(sample.kind)) return snapshot();
    const timestamp = Number.isFinite(sample.timestamp) ? sample.timestamp : now();
    samples.push(Object.freeze({
      timestamp,
      kind: sample.kind,
      value: clampNumber(sample.value, 0, 1, 0),
    }));
  ...
  const recommendBudget = (budget: RuntimeBudget): AdaptiveBudgetRecommendation => {
    ...
    let ratio = 1;
    if (summary.level === 'critical') ratio = 0.4;
    else if (summary.level === 'high') ratio = 0.6;
    else if (summary.level === 'elevated') ratio = 0.8;
    return Object.freeze({
      level: summary.level,
      ...
      cacheBytes: scaleInteger(budget.maxCacheBytes, Math.max(ratio, 0.5), 1_048_576),
