...
describe('runtimeHealthJournal', () => {
  ...
  it('computes a bounded rolling failure rate', () => {
    const journal = createRuntimeHealthJournal({ failureWindowSize: 4 }, FIXED_NOW);
  ...
  it('treats error severity as failure evidence even for non-failure kinds', () => {
    const journal = createRuntimeHealthJournal({}, FIXED_NOW);
  ...
  it('bounds messages and tag cardinality', () => {
    const journal = createRuntimeHealthJournal({ maxMessageLength: 5, maxTagsPerEvent: 2, maxTagLength: 4 }, FIXED_NOW);
    const event = journal.record({
      at: 1,
      kind: 'lifecycle',
      severity: 'info',
