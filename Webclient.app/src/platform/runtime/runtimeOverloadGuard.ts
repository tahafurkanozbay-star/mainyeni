...
export interface RuntimeOverloadSample {
  readonly lane: RuntimeFailureBudgetLane;
  readonly pressure: number;
}

export interface RuntimeOverloadLaneSnapshot {
  readonly lane: RuntimeFailureBudgetLane;
  readonly state: RuntimeOverloadState;
...
export interface RuntimeOverloadSnapshot {
  readonly sequence: number;
  readonly state: RuntimeOverloadState;
  readonly lanes: Readonly<Record<RuntimeFailureBudgetLane, RuntimeOverloadLaneSnapshot>>;
}

interface LaneState {
  samples: number[];
  state: RuntimeOverloadState;
  transitions: number;
}

const LANES: readonly RuntimeFailureBudgetLane[] = Object.freeze(['critical', 'interactive', 'background']);
const DEFAULT_POLICY: RuntimeOverloadPolicy = Object.freeze({
...
const integer = (value: number, name: string, maximum: number): number => {
...
const assertLane = (lane: RuntimeFailureBudgetLane): void => {
  if (!LANES.includes(lane)) throw new TypeError(`unsupported runtime overload lane: ${String(lane)}`);
};
...
export class RuntimeOverloadGuard {
  ...
  readonly #lanes: Record<RuntimeFailureBudgetLane, LaneState> = {
    critical: { samples: [], state: 'healthy', transitions: 0 },
    interactive: { samples: [], state: 'healthy', transitions: 0 },
    background: { samples: [], state: 'healthy', transitions: 0 },
  };
  #sequence = 0;

  constructor(policy: Partial<RuntimeOverloadPolicy> = {}) {
    this.#policy = normalizePolicy(policy);
  }

  record(sample: RuntimeOverloadSample): RuntimeOverloadLaneSnapshot {
    assertLane(sample.lane);
    const pressure = rate(sample.pressure, 'pressure');
    const lane = this.#lanes[sample.lane];
    lane.samples.push(pressure);
    const overflow = lane.samples.length - this.#policy.windowSize;
    if (overflow > 0) lane.samples.splice(0, overflow);
    const next = this.#classify(lane);
    if (next !== lane.state) {
      lane.state = next;
      lane.transitions += 1;
    }
    this.#sequence += 1;
    return this.lane(sample.lane);
  }

  recordMany(samples: readonly RuntimeOverloadSample[]): RuntimeOverloadSnapshot {
  ...
  lane(lane: RuntimeFailureBudgetLane): RuntimeOverloadLaneSnapshot {
    assertLane(lane);
    const value = this.#lanes[lane];
    let sum = 0;
    let peakPressure = 0;
    for (const pressure of value.samples) {
      sum += pressure;
    ...
    return Object.freeze({
      ...
      averagePressure: value.samples.length === 0 ? 0 : sum / value.samples.length,
      peakPressure,
      transitions: value.transitions,
    });
  }

  snapshot(): RuntimeOverloadSnapshot {
    const lanes = Object.freeze({
      critical: this.lane('critical'),
      interactive: this.lane('interactive'),
      background: this.lane('background'),
    });
    let state: RuntimeOverloadState = 'healthy';
    for (const lane of LANES) if (rank(lanes[lane].state) > rank(state)) state = lanes[lane].state;
    return Object.freeze({ sequence: this.#sequence, state, lanes });
  }

  policy(): RuntimeOverloadPolicy {
    return this.#policy;
  }
  ...
  reset(lane?: RuntimeFailureBudgetLane): void {
    if (lane !== undefined) {
      assertLane(lane);
      this.#lanes[lane].samples.length = 0;
      this.#lanes[lane].state = 'healthy';
      this.#lanes[lane].transitions = 0;
    } else {
      for (const current of LANES) {
        this.#lanes[current].samples.length = 0;
        this.#lanes[current].state = 'healthy';
        this.#lanes[current].transitions = 0;
      }
    }
    this.#sequence += 1;
  }

  #classify(lane: LaneState): RuntimeOverloadState {
    if (lane.samples.length < this.#policy.minimumSamples) return lane.state;
    const average = lane.samples.reduce((sum, value) => sum + value, 0) / lane.samples.length;
    if (lane.state !== 'healthy' && average <= this.#policy.recoveryThreshold) return 'healthy';
    if (average >= this.#policy.overloadThreshold) return 'overloaded';
    if (average >= this.#policy.pressureThreshold) return 'pressured';
    if (lane.state === 'overloaded') return 'pressured';
    return lane.state === 'pressured' ? 'pressured' : 'healthy';
  }
}
