...
interface QueueEntry {
  readonly id: string;
  readonly request: NormalizedAdmissionRequest;
...
export const createAdmissionController = (input: AdmissionPolicy, now: () => number = Date.now) => {
  ...
  const laneQueued = (lane: string): number => {
    ...
    for (const entry of queue) {
      if (entry.request.lane === lane) count += 1;
    }
    return count;
  };
  ...
  const canAdmit = (request: NormalizedAdmissionRequest): boolean => {
  ...
  const expireStale = (): void => {
    const timestamp = now();
    const stale = queue.filter((entry) => timestamp - entry.createdAt > policy.maxQueueAgeMs);
  ...
  function pump(): void {
    ...
    let entry = queue.find((candidate) => canAdmit(candidate.request));
    while (entry) {
      removeQueued(entry);
      entry.resolve(createLease(entry));
      entry = queue.find((candidate) => canAdmit(candidate.request));
    }
  }

  const acquire = (request: AdmissionRequest): Promise<AdmissionLease> => {
    if (disposed) {
      return Promise.reject(new AdmissionRejectedError('Admission controller is disposed.'));
    }
    const normalized = normalize(request);
    if (!normalized.key) {
      return Promise.reject(new AdmissionRejectedError('Admission key is required.'));
    }
    if (normalized.signal?.aborted) {
      return Promise.reject(normalized.signal.reason instanceof Error
        ? normalized.signal.reason
        : new AdmissionCancelledError());
    }
  ...
  const cancelQueued = (
    ...
    const matches = queue.filter((entry) => predicate(entry.request));
    let count = 0;
