...
export const createAdmissionController = (
  ...
  const canAdmit = (request: QueueEntry['request']): boolean => {
    ...
    return limit === undefined || laneActive(request.lane).length < positive(limit, policy.maxActive);
  };
  ...
  function pump(): void {
    if (disposed) return;
    expireStale();
    queue.sort((left, right) => priorityRank[left.request.priority] - priorityRank[right.request.priority] || left.createdAt - right.createdAt);
    let progressed = true;
    while (progressed) {
      progressed = false;
      for (let index = 0; index < queue.length; index += 1) {
        const entry = queue[index];
        if (!entry || !canAdmit(entry.request)) continue;
        queue.splice(index, 1);
        entry.cleanup?.();
  ...
  const acquire = (request: AdmissionRequest): Promise<AdmissionLease> => {
    ...
    return new Promise<AdmissionLease>((resolve, reject) => {
      const entry: QueueEntry = { id, request: normalized, createdAt: now(), resolve, reject };
