...
export const createAdmissionController = (input: AdmissionPolicy, now: () => number = Date.now) => {
  ...
  const normalize = (request: AdmissionRequest): QueueEntry['request'] => ({
    ...
    lane: request.lane?.trim() || 'default',
    priority: request.priority ?? 'normal',
    cost: Math.min(policy.maxCost, positive(request.cost ?? 1, 1)),
  ...
  const createLease = (entry: QueueEntry): AdmissionLease => {
    ...
    const lease: AdmissionLease = Object.freeze({
      ...
      cost: entry.request.cost,
      release: () => {
        if (released) return;
        released = true;
        if (active.delete(entry.id)) completed += 1;
        pump();
      },
    });
    active.set(entry.id, lease);
    admitted += 1;
    return lease;
  };
  ...
  function pump(): void {
    ...
    for (;;) {
      ...
      queue.splice(index, 1);
      entry.cleanup?.();
      entry.resolve(createLease(entry));
    }
  }
    ...
    return new Promise<AdmissionLease>((resolve, reject) => {
      ...
      if (normalized.signal) {
        const onAbort = (): void => {
          ...
          cancelled += 1;
          reject(normalized.signal?.reason instanceof Error ? normalized.signal.reason : new AdmissionCancelledError());
        };
  ...
  const cancelQueued = (predicate: (request: Readonly<AdmissionRequest>) => boolean = () => true): number => {
    ...
    for (let index = queue.length - 1; index >= 0; index -= 1) {
      ...
      queue.splice(index, 1);
      entry.cleanup?.();
      entry.reject(new AdmissionCancelledError());
      cancelled += 1;
      count += 1;
    }
    return count;
  ...
  const snapshot = (): AdmissionSnapshot => {
    ...
    for (const lease of active.values()) {
      const lane = lanes[lease.lane] ?? { active: 0, queued: 0, cost: 0 };
      lane.active += 1;
      lane.cost += lease.cost;
      lanes[lease.lane] = lane;
    }
    for (const entry of queue) {
      const lane = lanes[entry.request.lane] ?? { active: 0, queued: 0, cost: 0 };
      lane.queued += 1;
      lanes[entry.request.lane] = lane;
    }
