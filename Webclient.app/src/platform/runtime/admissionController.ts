...
const priorityRank: Readonly<Record<AdmissionPriority, number>> = Object.freeze({
  critical: 0,
  high: 1,
  normal: 2,
  background: 3,
...
export const createAdmissionController = (input: AdmissionPolicy, now: () => number = Date.now) => {
  ...
  const laneActiveCount = (lane: string): number => {
    ...
    for (const lease of active.values()) {
      if (lease.lane === lane) count += 1;
    }
    return count;
  };
  const laneQueued = (lane: string): number =>
    queue.reduce((count, entry) => count + Number(entry.request.lane === lane), 0);
  ...
  const acquire = (request: AdmissionRequest): Promise<AdmissionLease> => {
    ...
    if (normalized.signal?.aborted) {
      return Promise.reject(
        normalized.signal.reason instanceof Error ? normalized.signal.reason : new AdmissionCancelledError(),
      );
    }
    const laneQueueLimit = policy.laneMaxQueued[normalized.lane];
    if (
      queue.length >= policy.maxQueued
      || (laneQueueLimit !== undefined && laneQueued(normalized.lane) >= positive(laneQueueLimit, policy.maxQueued))
    ) {
      shed += 1;
  ...
  const cancelQueued = (
    predicate: (request: Readonly<AdmissionRequest>) => boolean = () => true,
  ): number => {
    let count = 0;
    for (let index = queue.length - 1; index >= 0; index -= 1) {
      const entry = queue[index];
