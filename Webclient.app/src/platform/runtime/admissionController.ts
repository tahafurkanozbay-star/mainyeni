...
export interface AdmissionLease { readonly id: string; readonly key: string; readonly lane: string; readonly cost: number; readonly release: () => void; }
export class AdmissionRejectedError extends Error { readonly code = 'PLATFORM_ADMISSION_SHED'; constructor(message = 'Runtime capacity is exhausted.') { super(message); this.name = 'AdmissionRejectedError'; } }
export class AdmissionCancelledError extends Error { readonly code = 'PLATFORM_ADMISSION_CANCELLED'; constructor(message = 'Admission request was cancelled.') { super(message); this.name = 'AbortError'; } }
interface QueueEntry { readonly id: string; readonly request: Required<Pick<AdmissionRequest, 'key' | 'lane' | 'priority' | 'cost'>> & Pick<AdmissionRequest, 'signal'>; readonly createdAt: number; readonly sequence: number; readonly resolve: (lease: AdmissionLease) => void; readonly reject: (reason: unknown) => void; cleanup?: () => void; }
...
export const createAdmissionController = (input: AdmissionPolicy, now: () => number = Date.now) => {
  ...
  const active = new Map<string, AdmissionLease>(); const queue: QueueEntry[] = []; let sequence = 0; let admitted = 0; let completed = 0; let cancelled = 0; let shed = 0; let expired = 0; let disposed = false;
  const nextId = (): string => `admission-${(++sequence).toString(36)}`;
  const laneActiveCount = (lane: string): number => { let count = 0; for (const lease of active.values()) if (lease.lane === lane) count += 1; return count; };
  const laneQueued = (lane: string): number => queue.reduce((count, entry) => count + Number(entry.request.lane === lane), 0);
  const activeCost = (): number => { let cost = 0; for (const lease of active.values()) cost += lease.cost; return cost; };
  const canAdmit = (request: QueueEntry['request']): boolean => { if (active.size >= policy.maxActive || activeCost() + request.cost > policy.maxCost) return false; const limit = policy.laneMaxActive[request.lane]; return limit === undefined || laneActiveCount(request.lane) < positive(limit, policy.maxActive); };
  ...
  const snapshot = (): AdmissionSnapshot => { expireStale(); const lanes: Record<string, { active: number; queued: number; cost: number }> = {}; for (const lease of active.values()) { const lane = lanes[lease.lane] ?? { active: 0, queued: 0, cost: 0 }; lane.active += 1; lane.cost += lease.cost; lanes[lease.lane] = lane; } for (const entry of queue) { const lane = lanes[entry.request.lane] ?? { active: 0, queued: 0, cost: 0 }; lane.queued += 1; lanes[entry.request.lane] = lane; } for (const value of Object.values(lanes)) Object.freeze(value); return Object.freeze({ active: active.size, queued: queue.length, activeCost: activeCost(), admitted, completed, cancelled, shed, expired, lanes: Object.freeze(lanes) }); };
