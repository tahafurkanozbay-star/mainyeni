...
export class RuntimeRecoveryPlanner {
  ...
  constructor(policy: Partial<RuntimeRecoveryPolicy> = {}) {
  ...
  evaluate(signal: RuntimeRecoverySignal): RuntimeRecoveryDecision {
    ...
    if (signal.overload === 'overloaded') {
      return this.#commit(signal.lane, 'drain', 'overload', now, this.#policy.overloadedCooldown);
    }
    if (currentPressure > 0) {
      return this.#commit(signal.lane, 'throttle', 'pressure', now, this.#policy.pressuredCooldown);
    }
  ...
  lane(lane: RuntimeFailureBudgetLane): RuntimeRecoveryLaneSnapshot {
    ...
    return Object.freeze({
      ...
      stableSamples: value.stableSamples,
  ...
  reset(lane?: RuntimeFailureBudgetLane): void {
    if (lane !== undefined) {
      assertLane(lane);
      this.#lanes[lane] = createLaneState();
    } else {
      for (const current of LANES) this.#lanes[current] = createLaneState();
  ...
  #decision(
    lane: RuntimeFailureBudgetLane,
    action: RuntimeRecoveryAction,
    reason: RuntimeRecoveryDecision['reason'],
    eligibleAt: number,
  ...
  #commit(
    laneName: RuntimeFailureBudgetLane,
    action: RuntimeRecoveryAction,
    reason: RuntimeRecoveryDecision['reason'],
    now: number,
    cooldown: number,
    ...
    lane.nextEligibleAt = now + cooldown;
    this.#sequence += 1;
