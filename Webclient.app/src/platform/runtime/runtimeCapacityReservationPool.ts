...
export type RuntimeReservationReason =
  | 'capacity'
  | 'queued'
  | 'duplicate'
  | 'queue-full'
...
export interface RuntimeCapacityReservationRequest {
  readonly id: string;
  readonly lane: RuntimeFailureBudgetLane;
  readonly priority: RuntimeReservationPriority;
...
export interface RuntimeCapacityReservationDecision {
  readonly id: string;
  readonly lane: RuntimeFailureBudgetLane;
  readonly disposition: RuntimeReservationDisposition;
  readonly reason: RuntimeReservationReason;
  readonly units: number;
  readonly at: number;
}

export interface RuntimeCapacityReservationLaneSnapshot {
  readonly lane: RuntimeFailureBudgetLane;
  readonly activeReservations: number;
  readonly activeUnits: number;
  readonly queuedReservations: number;
  readonly queuedUnits: number;
}

export interface RuntimeCapacityReservationSnapshot {
  readonly capacity: number;
  readonly activeReservations: number;
  readonly activeUnits: number;
  ...
  readonly lanes: Readonly<Record<RuntimeFailureBudgetLane, RuntimeCapacityReservationLaneSnapshot>>;
}

interface QueuedReservation extends RuntimeCapacityReservationRequest {
...
const DEFAULT_POLICY: RuntimeCapacityReservationPolicy = Object.freeze({
  capacity: 64,
  maximumReservationUnits: 32,
  maximumQueue: 128,
  maximumPerLaneQueue: 64,
  maximumHistory: 128,
  maximumLeaseDurationMs: 120_000,
  minimumCriticalReserve: 8,
  minimumInteractiveReserve: 8,
});
...
const assertId = (id: string): void => {
  if (typeof id !== 'string' || id.length === 0 || id.length > 256) throw new TypeError('reservation id must contain between 1 and 256 characters');
};
...
const assertLane = (lane: RuntimeFailureBudgetLane): void => {
  if (!LANES.includes(lane)) throw new TypeError(`unsupported reservation lane: ${String(lane)}`);
};
const assertPriority = (priority: RuntimeReservationPriority): void => {
  if (!(priority in PRIORITY_ORDER)) throw new TypeError(`unsupported reservation priority: ${String(priority)}`);
};
