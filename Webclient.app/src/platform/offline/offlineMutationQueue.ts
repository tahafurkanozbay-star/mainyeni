...
export class OfflineMutationQueue {
  ...
  #readyInsertionIndex(bucket: readonly QueueEntry[], entry: QueueEntry, low = 0, high = bucket.length): number { if (low >= high) return low; const middle = (low + high) >>> 1; const candidate = bucket[middle]; return candidate && this.#compareWithinPriority(candidate, entry) <= 0 ? this.#readyInsertionIndex(bucket, entry, middle + 1, high) : this.#readyInsertionIndex(bucket, entry, low, middle); }
  #queueReady(entry: QueueEntry): void { const bucket = this.#ready[entry.priority]; bucket.splice(this.#readyInsertionIndex(bucket, entry), 0, entry); }
  #takeReadyFromBucket(bucket: QueueEntry[]): QueueEntry | undefined { while (bucket.length > 0) { const candidate = bucket.shift(); if (candidate && candidate.state === 'queued' && this.#entries.get(candidate.descriptor.id) === candidate) return candidate; } return undefined; }
  #takeReady(): QueueEntry | undefined { return this.#takeReadyFromBucket(this.#ready.critical) ?? this.#takeReadyFromBucket(this.#ready.interactive) ?? this.#takeReadyFromBucket(this.#ready.background); }
  ...
  #drain(): void { if (!this.#online || !this.#executor || this.#disposed) return; this.pruneExpired(); this.#fillAvailableSlots(Math.max(0, this.maxConcurrent - this.#running)); }
  #fillAvailableSlots(remaining: number): void { if (remaining <= 0) return; const next = this.#takeReady(); if (!next) return; this.#start(next); this.#fillAvailableSlots(remaining - 1); }
