  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([mapConfigResult, configServicesResult]) => {
        ...
        try {
          const rawConfig = mapConfigResult?.data?.configValue;
          if (typeof rawConfig !== 'string') throw new Error('Map configuration is missing');
          MapManager.SetMapConfiguration(JSON.parse(rawConfig));
