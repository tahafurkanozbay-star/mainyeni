...
const loadQueryModules = () => {
  if (!queryModulesPromise) {
    queryModulesPromise = loadModules(['esri/tasks/QueryTask', 'esri/tasks/support/Query'])
      .catch((error) => {
...
const executeTask = async (options = {}, spatial = false) => {
  ...
  if (!url) throw Object.assign(new Error('A GIS query URL is required.'), { code: 'INVALID_GIS_URL' });
...
const execute = async (options = {}, spatial = false) => {
  try {
    // A caller-owned AbortSignal must remain isolated: sharing an in-flight
    // request would let one caller cancel work that another caller still needs.
    if (options.signal) return toLegacyResponse(await executeTask(options, spatial));

    const key = createRuntimeKey(options, spatial);
    if (inFlightQueries.has(key)) return await inFlightQueries.get(key);

    const work = executeTask(options, spatial)
      .then(toLegacyResponse)
      .catch(toErrorResponse)
      .finally(() => {
        if (inFlightQueries.get(key) === work) inFlightQueries.delete(key);
      });
...
export const GisQueryHelper = {
  ExecuteQuery: async (options = {}) => execute(options, false),
  ExecuteSpatialQuery: async (options = {}) => execute(options, true),
};
