...
- COMMIT / PR / MERGE DURUMU: branch work through `2902206e959d5bf33ff5d089cbf6828a90f495e9`; previous GIS PR #70 became stale/non-mergeable after main advanced and is superseded. Mandatory 4,000 meaningful-additions gate is not met; merge forbidden.
...
- TESTLER / BUILD / CI: focused Vitest regressions cover ArcGIS multi-page identity including OBJECTID=0, capability/stable-id fail-closed behavior, truncation and cancellation; scene tests cover accounting, invalid estimates, duplicates, per-layer caps, priority eviction, layer release, budget shrink, critical-resource preservation and clear. Exact-head GitHub Actions is authoritative; no PASS claimed until completed+success.
- NETWORK DEĞİŞİKLİKLERİ: none. No new endpoint, direct fetch, WMS/WFS/WMTS, CDN, analytics, telemetry, remote asset or dependency.
- GÜVENLİK KONTROLLERİ / DATA INTEGRITY: ArcGIS execution requires verified query readiness, pagination capability and stable identity; injected transport ownership remains unchanged. Scene estimates fail closed; budgets are bounded; duplicate IDs cannot double-count ownership; no secret/token introduced.
...
- MODERNİZASYON KARARLARI: compose existing strict ArcGIS executor/window primitives rather than duplicate transport; introduce renderer-agnostic scene admission accounting before binding budgets to concrete 2D/3D SDK adapters.
- PERFORMANS ETKİSİ: explicit CPU/GPU/draw-call/feature/resource caps bound scene pressure; lower-priority prefetch can yield to interactive work; per-layer caps prevent monopolization; no polling/timers.
