...
## Kent Rehberi PostGIS public API demo — 2026-09-18 13:35 TRT
- TUR / GÖREV: Data/API integration; mevcut .NET 10 User API içinde PostgreSQL/PostGIS tabanlı, reverse-proxy/same-origin GeoJSON Kent Rehberi servisi.
- BASE MAIN: `cfbeaec827da3b62c9decddb68964fcd8ea7ac13`; son refresh'te current `main` aynı SHA ve branch `behind=0`, merge-base exact current main.
- BRANCH / PR: `agent/data-api-20260918-1314-cfbeaec`, draft PR #129 `feat(data): add secure PostGIS Kent Rehberi GeoJSON API`.
- CODE HEAD before this progress commit: `be1ddc034ad024d459f301d7d8833c6f7fcb78b0`; base...head = 1,968 additions / 1 deletion / 14 files. Repository'nin ~4,000 meaningful-additions merge hedefi/gate'i karşılanmadığı için PR draft tutuluyor ve merge edilmiyor.
- MİMARİ KARARI: ikinci Go/Node/Python runtime eklemek yerine mevcut .NET 10/C#14, Npgsql, merkezi CORS/rate-limit/request-timeout/security-header/health/compression altyapısı kullanıldı. Böylece yeni deployment/secret/runtime yüzeyi oluşturulmadı.
- API SÖZLEŞMESİ: `GET /kent-rehberi`, `/{objectId}`, `/nearby`, `/capabilities`; reverse proxy ile public hedef `/api/kent-rehberi`. Filtreler: ilce, mahalle, tur, q, bbox, objectid keyset cursor; nearby sorgusu EPSG:4326 koordinattan metre bazlı PostGIS geography mesafesi kullanıyor.
- DATA / SECURITY: tablo adı server-owned sabit; kullanıcı filtreleri Npgsql parameter; q wildcard karakterleri literal escape; limit/radius/query/bbox sınırları fail-closed; cancellation + connection/command timeout; generic 503; `gdb_geomattr_data` sorgulanmıyor ve response'a çıkmıyor. Tracked config'te gerçek host/veritabanı/parola yok; runtime secret `ConnectionStrings__KentRehberi`.
- AUTHORIZATION EVIDENCE: public endpointlerin anonim tasarımı her action üzerinde açık `[AllowAnonymous]` ile işaretlendi ve data-minimization/security contract testleriyle desteklendi. İlk Release QA exact-base gate'in bulduğu tek HIGH regression bu eksik yerel anonimlik kanıtıydı; `be1ddc0...` ile giderildi.
- DATABASE RUNBOOK: `database/kent-rehberi-api.sql` yalnız gerekli kolonlara SELECT yetkili rol, geometry GiST, geography-expression GiST, objectid/admin filter indexleri, duplicate preflight ve ANALYZE yönergelerini içeriyor; credential/host gömülü değil.
- TESTLER / BUILD / CI: feature head `be1ddc0...` üzerinde Platform Architecture Audit completed+success; Platform Backend Validation restore/audit/build/xUnit/User+Admin publish completed+success; Release QA typed-release exact-base regression gate completed+success; Release QA backend build/xUnit/publish completed+success. Webclient release validationın tüm substantive lint/TypeScript/Vitest/native-tooling/production-build/integrity/budget adımları completed+success; yalnız workflow post-cleanup devam ederken bu progress kaydı yazıldı. Bu progress-only commit yeni exact head oluşturacağı için merge-ready PASS iddiası yapılmıyor; post-progress exact-head workflows ayrıca doğrulanmalı.
- NETWORK DEĞİŞİKLİKLERİ: yeni public DB portu yok; tarayıcı PostgreSQL host/port görmüyor. Aynı-origin reverse proxy kullanılıyor. WMS/WFS, analytics, telemetry, CDN veya remote asset eklenmedi.
- PERFORMANS ETKİSİ: default 500 / max 2000 result bound; bbox viewport query; keyset pagination; short public cache headers; geometry/geography spatial index runbook; pooled bounded DB bağlantısı. Substring q için extension zorunlu tutulmadı; gerçek profiling gerekirse pg_trgm ayrı DBA kararı.
- İKON EŞLEŞTİRME: bu backend demosu ikon resolver/config davranışını değiştirmiyor; mevcut ortak deterministic ikon authority korunuyor.
- ÇÖZÜLEN HATALAR: endpoint authorization evidence regression giderildi; binary GDB metadata exposure engellendi; doğrudan DB/browser bağlantısı yerine secretsız tracked-config + server-side proxy boundary kuruldu.
- KALAN SORUNLAR / SONRAKİ GÖREV: gerçek production DB credentials bu branch'e verilmediği için canlı DB smoke testi yapılmadı. Deployment ortamında readonly rol/index runbook uygulanmalı, `ConnectionStrings__KentRehberi` secret tanımlanmalı, küçük bbox/nearby/query smoke testleri ve EXPLAIN planları doğrulanmalı. PR #129 meaningful-additions gate altında olduğundan merge edilmemeli; ek Data/API işi gerekirse aynı kanonik PR'da gerçek kapsamla ilerletilmeli, fakat current main ilerlerse lifecycle kurallarına göre fresh-current-main recovery uygulanmalı.

## Kent Rehberi PostGIS API hardening continuation — 2026-09-18
- TUR / GÖREV: PR #129 Data/API hardening; ayrı PostGIS datasource readiness, fail-closed pagination integrity ve public query abuse-resistance.
- BASE / BRANCH: base `cfbeaec827da3b62c9decddb68964fcd8ea7ac13`; `agent/data-api-20260918-1314-cfbeaec`; bu kayıt öncesi code/doc head `7597959a4e2b850e84631953ab922512f59bf655`.
- READINESS: `KentRehberiHealthCheck` eklendi ve `ApiPlatformDefaults.ReadinessTag` ile mevcut `/health/ready` zincirine bağlandı. Feature enabled iken `ConnectionStrings__KentRehberi` eksikse, bağlantı açılamıyorsa veya sabit kaynak tablo okunamıyorsa readiness unhealthy; public health response exception/host/SQL ayrıntısı taşımıyor.
- CURSOR DATA INTEGRITY: kaynak DDL `objectid` için UNIQUE constraint göstermediği için `ObjectIdCursorEnabled=false` tracked default yapıldı. `afterObjectId` istekleri persistent ObjectID uniqueness unique index/constraint ile doğrulanıp deployment flag explicit açılana kadar 400 ile fail-closed. Capability response cursor durumunu bildiriyor; disabled iken next cursor üretilmiyor.
- GÜVENLİK / OBSERVABILITY: controller DB failure loglarında exception object/message/stack attach edilmiyor; yalnız güvenli failure type ve trace id yazılıyor. Connection/command timeout, bounded pool, Npgsql error-detail suppression ve generic public 503 davranışı korunuyor.
- PERFORMANS / ABUSE RESISTANCE: public `MaxLimit` hard ceiling 2.000, `MaxRadiusMeters` hard ceiling 50.000 m; deployment config bu repository güvenlik tavanlarını büyütemiyor. Substring `q` araması minimum 2 / maksimum 120 karakter; tek karakterli pahalı `ILIKE '%q%'` taramaları reddediliyor.
- DBA RUNBOOK: duplicate preflight tek başına cursor açmak için yeterli sayılmıyor; cursor etkinleştirmeden önce ObjectID benzersizliğinin unique index/constraint ile kalıcı garanti altına alınması isteniyor.
- TESTLER: query validation suite cursor-disabled, hard ceiling, min-query ve health option sınırlarını kapsıyor; yeni health tests disabled/unconfigured/readiness-registration davranışını; security contract tests readiness registration, safe failure logging ve tracked cursor defaultunu kapsıyor.
