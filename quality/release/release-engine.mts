...
export async function runReleaseEngine(inventory: RepositoryInventory, context: ReleaseContext, options: ReleaseEngineOptions = {}, baseline?: BaselineSnapshot): Promise<ReleaseExecution> {
  ...
  const sections: AuditSection<unknown>[] = [auditModernization(inventory), scanSource(inventory), auditDependencies(inventory), auditNetwork(inventory), auditGis(inventory), auditUx(inventory), auditAccessibility(inventory), auditPerformance(inventory), auditTestContracts(inventory)];
