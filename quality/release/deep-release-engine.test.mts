...
test('release engine exposes every deep QA section in deterministic order', async () => {
  ...
  const expected = [
    ...
    'Whole-code source regression audit',
    auditSecurity(fixtureInventory([])).title,
    auditBackendSecurity(fixtureInventory([])).title,
    'Dependency modernization and supply-chain audit',
    'Network privacy and transport audit',
    '2D/3D GIS, service, icon and lifecycle audit',
    auditGisReleaseContracts(fixtureInventory([])).title,
    'Responsive and interaction quality audit',
    ...
    'Regression and release-test contract audit',
  ];
