...
function jsonFindings(files: readonly SourceFile[]): { findings: Finding[]; failures: number } {
  ...
  for (const file of files) {
    ...
    try {
      ...
      failures += 1;
      findings.push({
        id: 'data-invalid-json', domain: 'data', severity: 'critical', blocking: true,
        title: 'Invalid runtime JSON', message: 'Runtime JSON must parse deterministically before release.',
        location: { file: file.repositoryPath, line: 1 },
        evidence: { value: error instanceof Error ? error.message : String(error) },
        remediation: 'Repair the JSON document and add a schema/fixture regression test.', tags: ['json', 'schema', 'release'],
      });
    }
  }
...
function regexSignals(file: SourceFile, pattern: RegExp, id: string, severity: 'low' | 'medium' | 'high', title: string, message: string, remediation: string, max = 5): Finding[] {
  ...
  while ((match = matcher.exec(file.text)) !== null && findings.length < max) {
    findings.push({ id, domain: 'data', severity, title, message,
      location: { file: file.repositoryPath, line: lines.lineAt(match.index) },
      evidence: { excerpt: snippetAround(file.text, match.index, 120) }, remediation, tags: ['data-integrity'] });
...
function collectionFindings(file: SourceFile): Finding[] {
  ...
  const lines = createLineIndex(file.text);
  return [{
    id: 'data-unbounded-runtime-collection', domain: 'data', severity: 'medium',
    title: 'Potentially unbounded runtime collection',
    message: 'Repeated collection growth without an obvious capacity/eviction hint can amplify large GIS/data workloads.',
    location: { file: file.repositoryPath, line: lines.lineAt(first.index ?? 0) },
    evidence: { value: matches.length, excerpt: snippetAround(file.text, first.index ?? 0, 120) },
    remediation: 'Introduce an explicit capacity/budget, eviction policy, pagination/windowing, or document why lifetime is inherently bounded.',
    tags: ['memory', 'large-data', 'performance'],
  }];
}
...
export function auditDataIntegrity(inventory: RepositoryInventory): AuditSection<DataIntegritySummary> {
  ...
  return {
    domain: 'data', title: 'Runtime data integrity and boundedness audit',
    summary: { jsonFiles: jsonFiles.length, parseFailures: parsed.failures, duplicateIdentitySignals, unsafeNumericIdentitySignals, unboundedCollectionSignals, findings: sorted },
