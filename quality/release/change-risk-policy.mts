...
const DATA_PATH = /(?:^|\/)(?:data|repository|store|cache|dataset|registry|model|entity|dto|schema)(?:[._/-]|$)/iu;
...
const RESPONSIVE_PATH = /(?:^|\/)(?:responsive|layout|viewport|breakpoint|mobile|drawer|sidebar)(?:[._/-]|$)/iu;
...
const POLICIES: readonly ChangeRiskPolicy[] = Object.freeze([
  {
    area: 'security',
    domain: 'security',
    uncoveredSeverity: 'high',
    title: 'Security-sensitive change lacks focused regression evidence',
    description: 'Authentication, authorization, token/session, CORS, credential and unsafe-sink changes require focused security validation.',
    pathPatterns: [SECURITY_PATH],
    contentPatterns: [SECURITY_CONTENT],
    testPatterns: [SECURITY_TEST],
    validationPatterns: [RELEASE_TEST, CI_TEST],
    requiresFocusedEvidence: true,
  },
  {
    area: 'backend-api',
    domain: 'security',
    uncoveredSeverity: 'high',
    title: 'Backend API change lacks focused regression evidence',
    description: 'Controller, endpoint and outbound HTTP changes require backend contract or security tests.',
    pathPatterns: [BACKEND_API_PATH],
    contentPatterns: [BACKEND_API_CONTENT],
    testPatterns: [BACKEND_TEST],
    validationPatterns: [WORKFLOW_VALIDATION],
    requiresFocusedEvidence: true,
  },
  {
    area: 'gis',
    domain: 'gis',
    uncoveredSeverity: 'high',
    title: 'GIS runtime change lacks focused regression evidence',
    description: '2D/3D GIS, ArcGIS, layer, geometry, query and viewport changes require focused GIS regression coverage.',
    pathPatterns: [GIS_PATH],
    contentPatterns: [GIS_CONTENT],
    testPatterns: [GIS_TEST],
    validationPatterns: [WORKFLOW_VALIDATION, RELEASE_TEST],
    requiresFocusedEvidence: true,
  },
  {
    area: 'data',
    domain: 'data',
    uncoveredSeverity: 'high',
    title: 'Data-model or repository change lacks regression evidence',
    description: 'Repository, store, cache, dataset and schema changes need focused integrity coverage.',
    pathPatterns: [DATA_PATH],
    testPatterns: [DATA_TEST],
    validationPatterns: [RELEASE_TEST],
    requiresFocusedEvidence: true,
  },
  {
    area: 'search',
    domain: 'search',
    uncoveredSeverity: 'high',
    title: 'Search/address change lacks focused regression evidence',
    description: 'Search, address, geocoding and suggestion behavior changes require focused regression coverage.',
    pathPatterns: [SEARCH_PATH],
    testPatterns: [SEARCH_TEST],
    validationPatterns: [WORKFLOW_VALIDATION],
    requiresFocusedEvidence: true,
  },
  {
    ...
    domain: 'architecture',
    uncoveredSeverity: 'medium',
    title: 'Frontend runtime change lacks adjacent tests',
    description: 'Runtime frontend changes should be accompanied by changed tests unless a more focused area policy already supplies evidence.',
    pathPatterns: [FRONTEND_PATH],
    testPatterns: [FRONTEND_TEST],
    validationPatterns: [WORKFLOW_VALIDATION],
    requiresFocusedEvidence: true,
  },
  {
    ...
    uncoveredSeverity: 'high',
    title: 'Accessibility-sensitive UI change lacks focused tests',
    description: 'Focus, keyboard, ARIA, modal surface and reduced-motion behavior require explicit accessibility regression evidence.',
    pathPatterns: [ACCESSIBILITY_PATH],
    contentPatterns: [ACCESSIBILITY_CONTENT],
    testPatterns: [ACCESSIBILITY_TEST],
    validationPatterns: [RELEASE_TEST],
    requiresFocusedEvidence: true,
  },
  {
    ...
    domain: 'responsive',
    uncoveredSeverity: 'medium',
    title: 'Responsive/layout change lacks focused tests',
    description: 'Viewport, breakpoint and adaptive layout changes should have focused responsive regression evidence.',
    pathPatterns: [RESPONSIVE_PATH],
    contentPatterns: [RESPONSIVE_CONTENT],
    testPatterns: [RESPONSIVE_TEST],
    validationPatterns: [RELEASE_TEST],
    requiresFocusedEvidence: true,
  },
  {
    ...
    domain: 'dependencies',
    uncoveredSeverity: 'high',
    title: 'Dependency change lacks lockfile or dependency validation evidence',
    description: 'Dependency graph mutations require deterministic lockfile and validation evidence.',
    pathPatterns: [DEPENDENCY_PATH],
    testPatterns: [DEPENDENCY_TEST],
    validationPatterns: [WORKFLOW_VALIDATION, RELEASE_TEST, PLATFORM_VALIDATION],
    requiresFocusedEvidence: false,
  },
  {
    area: 'ci',
    domain: 'build',
    uncoveredSeverity: 'high',
    title: 'CI workflow change lacks adversarial contract coverage',
    description: 'GitHub Actions changes require workflow security/reliability tests or typed release validation updates.',
    pathPatterns: [CI_PATH],
    testPatterns: [CI_TEST, RELEASE_TEST],
    validationPatterns: [RELEASE_TOOL_VALIDATION],
    requiresFocusedEvidence: true,
  },
  {
    ...
    domain: 'release',
    uncoveredSeverity: 'high',
    title: 'Release tooling change lacks direct regression tests',
    description: 'Changes to release gate implementation must update or add typed release tests.',
    pathPatterns: [RELEASE_VALIDATION_PATH],
    testPatterns: [RELEASE_TEST, CI_TEST, PLATFORM_VALIDATION],
    validationPatterns: [WORKFLOW_VALIDATION],
    requiresFocusedEvidence: true,
  },
  {
    ...
    domain: 'data',
    uncoveredSeverity: 'high',
    title: 'Database or migration change lacks regression evidence',
    description: 'Schema, migration and SQL changes require focused database/repository validation.',
    pathPatterns: [DATABASE_PATH],
    contentPatterns: [DATABASE_CONTENT],
    testPatterns: [DATABASE_TEST],
    validationPatterns: [WORKFLOW_VALIDATION],
    requiresFocusedEvidence: true,
  },
  {
    ...
    uncoveredSeverity: 'medium',
    title: 'Observability change lacks focused regression evidence',
    description: 'Logging, metrics, tracing and correlation changes should preserve bounded diagnostics and privacy contracts.',
    pathPatterns: [OBSERVABILITY_PATH],
    contentPatterns: [OBSERVABILITY_CONTENT],
    testPatterns: [OBSERVABILITY_TEST],
    validationPatterns: [RELEASE_TEST],
    requiresFocusedEvidence: true,
  },
  {
    ...
    domain: 'performance',
    uncoveredSeverity: 'medium',
    title: 'Performance-sensitive change lacks focused evidence',
    description: 'Scheduling, caching, queueing and hot-path changes should include focused performance/budget regression evidence.',
    pathPatterns: [PERFORMANCE_PATH],
    contentPatterns: [PERFORMANCE_CONTENT],
    testPatterns: [PERFORMANCE_TEST],
    validationPatterns: [WORKFLOW_VALIDATION, RELEASE_TEST],
    requiresFocusedEvidence: true,
  },
  {
    ...
    domain: 'architecture',
    uncoveredSeverity: 'medium',
    title: 'Configuration/toolchain change lacks validation evidence',
    description: 'Runtime/build configuration changes should include validation or contract coverage.',
    pathPatterns: [CONFIG_PATH],
    testPatterns: [CONFIG_TEST, RELEASE_TEST],
    validationPatterns: [WORKFLOW_VALIDATION, PLATFORM_VALIDATION],
    requiresFocusedEvidence: true,
  },
]);
...
export function isTestPath(input: string): boolean {
  return TEST_PATH.test(normalizeRepositoryPath(input));
}

export function isDocumentationPath(input: string): boolean {
  return DOCUMENTATION_PATH.test(normalizeRepositoryPath(input));
}

export function isGeneratedPath(input: string): boolean {
  return GENERATED_PATH.test(normalizeRepositoryPath(input));
}

export function isWorkflowPath(input: string): boolean {
  return WORKFLOW_PATH.test(normalizeRepositoryPath(input));
}

export function isPackageManifestPath(input: string): boolean {
  return PACKAGE_MANIFEST.test(normalizeRepositoryPath(input));
}

export function isPackageLockPath(input: string): boolean {
  return PACKAGE_LOCK.test(normalizeRepositoryPath(input));
}

export function isReleaseValidationPath(input: string): boolean {
  const path = normalizeRepositoryPath(input);
  return RELEASE_VALIDATION_PATH.test(path) || WORKFLOW_VALIDATION.test(path) || PLATFORM_VALIDATION.test(path);
}

export function isReleaseCriticalValidationPath(input: string): boolean {
  const path = normalizeRepositoryPath(input);
  return RELEASE_CRITICAL_VALIDATION_PATHS.some(pattern => pattern.test(path));
}

export function isProductionCandidatePath(input: string): boolean {
  const path = normalizeRepositoryPath(input);
  if (isGeneratedPath(path) || isDocumentationPath(path) || isTestPath(path)) return false;
  if (path.startsWith('.github/')) return false;
  return SOURCE_EXTENSION.test(path);
}

export function pathNature(input: string): PathNature {
  const path = normalizeRepositoryPath(input);
  return {
    path,
    ...
    workflow: isWorkflowPath(path),
    manifest: isPackageManifestPath(path),
    lockfile: isPackageLockPath(path),
    releaseValidation: isReleaseValidationPath(path),
    productionCandidate: isProductionCandidatePath(path),
  };
}

export function policyForArea(area: ChangeRiskArea): ChangeRiskPolicy {
  const policy = POLICIES.find(candidate => candidate.area === area);
  if (!policy) throw new Error(`Unknown change-risk area: ${area}`);
  return policy;
}
...
export function testSupportsArea(input: string, area: ChangeRiskArea): boolean {
  const path = normalizeRepositoryPath(input);
  if (!isTestPath(path)) return false;
  return matchesAny(path, policyForArea(area).testPatterns);
}

export function validationSupportsArea(input: string, area: ChangeRiskArea): boolean {
  const path = normalizeRepositoryPath(input);
  const patterns = policyForArea(area).validationPatterns ?? [];
  return matchesAny(path, patterns);
}

export function evidenceSupportsArea(input: string, area: ChangeRiskArea): boolean {
  return testSupportsArea(input, area) || validationSupportsArea(input, area);
}

export function workspaceDirectory(input: string): string {
  const path = normalizeRepositoryPath(input);
  const index = path.lastIndexOf('/');
  return index < 0 ? '' : path.slice(0, index);
}

export function packageLockCandidates(manifestPath: string): readonly string[] {
  const directory = workspaceDirectory(manifestPath);
...
function dependencyMap(value: unknown): Readonly<Record<string, string>> {
...
function manifestDependencies(text: string): { readonly ok: boolean; readonly values: Readonly<Record<string, string>> } {
  const parsed = safeJsonParse<Record<string, unknown>>(text);
  if (!parsed.ok || !parsed.value) return { ok: false, values: {} };
  const result: Record<string, string> = {};
  for (const field of ['dependencies', 'devDependencies', 'optionalDependencies', 'peerDependencies'] as const) {
    for (const [name, version] of Object.entries(dependencyMap(parsed.value[field]))) {
...
export function compareManifestDependencies(baselineText: string, currentText: string): DependencyChangeSummary {
  const baseline = manifestDependencies(baselineText);
  const current = manifestDependencies(currentText);
  const names = new Set([...Object.keys(baseline.values), ...Object.keys(current.values)]);
  const added: string[] = [];
  const removed: string[] = [];
  const versionChanged: string[] = [];
  for (const name of names) {
    const before = baseline.values[name];
    const after = current.values[name];
    if (before === undefined && after !== undefined) added.push(name);
    else if (before !== undefined && after === undefined) removed.push(name);
    else if (before !== after) versionChanged.push(name);
  }
  added.sort((left, right) => left.localeCompare(right, 'en'));
  removed.sort((left, right) => left.localeCompare(right, 'en'));
  versionChanged.sort((left, right) => left.localeCompare(right, 'en'));
  return {
    changed: added.length > 0 || removed.length > 0 || versionChanged.length > 0,
    added,
    removed,
    versionChanged,
    invalidBaseline: !baseline.ok,
    invalidCurrent: !current.ok,
  };
}

export function areaEvidencePaths(paths: readonly string[], area: ChangeRiskArea): readonly string[] {
  return paths
    .map(normalizeRepositoryPath)
    .filter(path => evidenceSupportsArea(path, area))
    .sort((left, right) => left.localeCompare(right, 'en'));
}

export function productionPathsForArea(
  files: readonly { readonly path: string; readonly text: string }[],
  area: ChangeRiskArea,
): readonly string[] {
  return files
    .filter(file => classifyRiskAreas(file.path, file.text).includes(area))
