    ...
    if (JAVASCRIPT_EXTENSIONS.has(extension)) {
      ...
      if (
        ...
        findings.push(finding(
          ...
          relative,
          'JavaScript outside the Business migration tranche and bounded transitional allowlist is forbidden.',
        ));
      }
      continue;
    }
...
export const formatTypedSourceBoundary = (report) => {
  const lines = [
    ...
    `- Transitional JavaScript files: **${report.summary.transitionalJavascriptFiles}**`,
  ...
  lines.push(
    'Business JavaScript is intentionally excluded from this boundary because it is owned by a concurrent migration tranche. '
    + 'The two transitional allowlist entries are bounded existing files owned by concurrent GIS/Experience work. '
    + 'All other application source areas are permanently ratcheted away from JavaScript.',
    '',
  );
  return lines.join('\n');
};
